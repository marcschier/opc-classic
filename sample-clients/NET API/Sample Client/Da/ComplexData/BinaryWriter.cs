...
{	
	...
	public class BinaryWriter : BinaryStream
	{
		...
		public  byte[] Write(ComplexValue namedValue, TypeDictionary dictionary, string typeName)
		{
			...
			if (bytesRequired == 0)
			{
				throw new InvalidDataToWrite("Could not write value into buffer.");
			}
		...
		private int WriteType(Context context, ComplexValue namedValue)
		{
			...
			{
				throw new InvalidDataToWrite("Type instance does not contain field values.");
			}
			...
			{
				throw new InvalidDataToWrite("Type instance does not contain the correct number of fields.");
			}
			...
			for (int ii = 0; ii < type.Field.Length; ii++)
			{
				...
				{
					throw new InvalidDataToWrite("Could not write field '" + field.Name + "' in type '" + type.TypeID +"'.");
				}
		...
		private int WriteField(Context context, TypeReference field, object fieldValue)
		{
			...
			{
				throw new InvalidSchema("Reference type '" + field.TypeID + "' not found.");
			}

			if (fieldValue.GetType() != typeof(ComplexValue))
			{
				throw new InvalidDataToWrite("Instance of type is not the correct type.");
			}		
		...
		private int WriteField(Context context, Integer field, object fieldValue)
		{
			...
			if (buffer != null)
			{
				...
				{
					throw new InvalidDataToWrite("Unexpected end of buffer.");
				}
		...
		)
		{
			...
			if (charCount == -1)
			{
				...
				if (charWidth > 2)
				{
					...
					{
						throw new InvalidDataToWrite("Field value is not a byte array.");
					}
				...
				else
				{
					...
					{
						throw new InvalidDataToWrite("Field value is not a string.");
					}
		...
		private int WriteField(Context context, BitString field, object fieldValue, ref byte bitOffset)
		{
			...
			if (fieldValue.GetType() != typeof(byte[]))
			{
				throw new InvalidDataToWrite("Wrong data type to write.");
			}
		...
		)
		{
			...
			{
				throw new InvalidDataToWrite("Array field value is not an array type.");
			}
		...
		)
		{
			...
			if (namedValue == null)
			{
				throw new InvalidSchema("Referenced field not found (" + fieldName + ").");
			}
			...
			{	
				throw new InvalidDataToWrite("Reference field value and the actual array length are not equal.");
			}
		}
	}
}
