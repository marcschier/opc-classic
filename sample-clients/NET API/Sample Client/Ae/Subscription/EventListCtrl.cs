//============================================================================
// TITLE: EventListCtrl.cs
//
// CONTENTS:
// 
// A control used to display events reported from the server.
//
...
namespace Opc.Ae.SampleClient
{
	...
	public class EventListCtrl : System.Windows.Forms.UserControl
	{
		...
		Opc.Ae.Server m_server = null;
		Hashtable m_subscriptions = new Hashtable();
		#endregion

		private class State
		{
			public Opc.Ae.Subscription      Subscription;
			public EventChangedEventHandler EventHandler;
		}
		...
		/// </summary>
		public void AddSubscription(Opc.Ae.Subscription subscription)
		{
			...
			state.EventHandler = new EventChangedEventHandler(Subscription_EventChanged);

			subscription.EventChanged += new EventChangedEventHandler(Subscription_EventChanged);
		...
		/// </summary>
		public void RemoveSubscription(Opc.Ae.Subscription subscription)
		{
			...
			if (state != null)
			{
				...
				subscription.EventChanged -= state.EventHandler;
				...
				{
					EventNotification notification = (EventNotification)item.Tag;
		...
		public void RemoveSubscriptions()
		{
			...
			{
				try   { state.Subscription.EventChanged -= state.EventHandler; }
				catch {}
			}
		...
		/// </summary>
		public void Add(EventNotification notification, bool refresh)
		{
			...
			item.ImageIndex = (notification.EventType != EventType.Condition)?Resources.IMAGE_YELLOW_SCROLL:Resources.IMAGE_GREEN_SCROLL;
		...
		private void NotificationsLV_DoubleClick(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (typeof(BrowseElement).IsInstanceOfType(NotificationsLV.SelectedItems[0].Tag))
				{
				}
			}
		...
		private void Subscription_EventChanged(EventNotification[] notifications, bool refresh, bool lastRefresh)
		{
			// synchronize the HMI thread.
			if (InvokeRequired)
			{
				BeginInvoke(new EventChangedEventHandler(Subscription_EventChanged), new object[] {notifications, refresh, lastRefresh });
                return;
			}
		...
		private void ViewMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				EventNotification notification = (EventNotification)NotificationsLV.SelectedItems[0].Tag;
		...
		private void AcknowledgeMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				EventNotification[] notifications = new EventNotification[NotificationsLV.SelectedItems.Count];

				for (int ii = 0; ii < notifications.Length; ii++)
				{
					notifications[ii] = (EventNotification)NotificationsLV.SelectedItems[ii].Tag;
				}
