...
namespace Opc.Ae.SampleClient
{
	...
	public class AcknowledgerEditDlg : System.Windows.Forms.Form
	{
		...
		/// </summary>
		public bool ShowDialog(Opc.Ae.Server server, EventNotification[] notifications)
		{
			...
			try
			{
				...
				EventAcknowledgement[] acknowledgements = new EventAcknowledgement[notifications.Length];

				for (int ii = 0; ii < notifications.Length; ii++)
				{					
					acknowledgements[ii] = new EventAcknowledgement(notifications[ii]);
				}

				// acknowledge events.
				ResultID[] results = server.AcknowledgeCondition(
					AcknowledgerTB.Text,
					CommentTB.Text,
					acknowledgements);
