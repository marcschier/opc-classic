...
namespace Opc.Ae.SampleClient
{
	...
	public class SubscriptionEditDlg : System.Windows.Forms.Form
	{
		...
		private Opc.Ae.SampleClient.AttributesCtrl AttributesCTRL;
		private Opc.Ae.SampleClient.SubscriptionStateEditCtrl StateCTRL;
		private Opc.Ae.SampleClient.SubscriptionFiltersEditCtrl FiltersCTRL;
		private System.Windows.Forms.GroupBox StateGB;
		private System.Windows.Forms.GroupBox FiltersGB;
		private Opc.Ae.SampleClient.CategoriesCtrl CategoriesCTRL;
		private Opc.Ae.SampleClient.BrowseCtrl BrowseCTRL;
		private System.Windows.Forms.Panel StateFiltersPN;
		private Opc.Ae.SampleClient.AreaSourceListCtrl AreaSourcesListCTRL;
		/// <summary>
		...
		private void InitializeComponent()
		{
			...
			this.AreaSourcesListCTRL = new Opc.Ae.SampleClient.AreaSourceListCtrl();
			this.AttributesCTRL = new Opc.Ae.SampleClient.AttributesCtrl();
			this.StateFiltersPN = new System.Windows.Forms.Panel();
			this.CategoriesCTRL = new Opc.Ae.SampleClient.CategoriesCtrl();
			this.FiltersGB = new System.Windows.Forms.GroupBox();
			this.FiltersCTRL = new Opc.Ae.SampleClient.SubscriptionFiltersEditCtrl();
			this.StateGB = new System.Windows.Forms.GroupBox();
			this.StateCTRL = new Opc.Ae.SampleClient.SubscriptionStateEditCtrl();
			...
			this.CategoriesCTRL.CategoryChecked += new Opc.Ae.SampleClient.CategoriesCtrl.CategoryCheckedEventHandler(this.CategoriesCTRL_CategorySelected);
			// 
			// FiltersGB
			// 
			...
			this.BrowseCTRL.NodeSelected += new Opc.Ae.SampleClient.BrowseCtrl.NodeSelectedEventHandler(this.BrowseCTRL_NodeSelected);
			// 
			// ButtonsPN
			// 
		...
		/// </summary>
		public Subscription ShowDialog(Opc.Ae.Server server, Opc.Ae.Subscription subscription)
		{
		...
		private void NextBTN_Click(object sender, System.EventArgs e)
		{
			if (m_stage == 0)
			{
				m_state   = (SubscriptionState)StateCTRL.Get();
				m_filters = (SubscriptionFilters)FiltersCTRL.Get();
		...
		{
            bool deleteOnError = m_subscription == null;
			...
			try
			{
				...
				if (m_subscription == null)
				{
					...
					m_subscription = (Opc.Ae.Subscription)m_server.CreateSubscription(m_state);
				}

				// update existing subscription.
				else
				{
					m_subscription.ModifyState(((int)StateMask.All & ~(int)StateMask.Active), m_state); 
				}
				...
				if (active || update)
				{
					...
					m_subscription.ModifyState((int)StateMask.Active, m_state); 
				}
		...
		/// </summary>
		private void BrowseCTRL_NodeSelected(Opc.Ae.BrowseElement element, bool picked)
		{
			if (picked)
			{
