...
{	
	...
	public class TestEditDlg : System.Windows.Forms.Form
	{
		...
		/// </summary>
		public bool ShowDialog(Opc.Hda.Server server, TestData test)
		{
			...
			{
				// add expected result.
				ResultIDTB.Text = test.Expected.ResultID.Name.Name;
			...
			if (test.Expected != null)
			{
				...
				{
					test.Expected.ResultID = new ResultID(ResultIDTB.Text, "");
				}
				...
				{
					if (typeof(ItemValue).IsInstanceOfType(listItem.Tag))
					{
		...
		/// </summary>
		private void AddItem(ItemValue item, bool highlight)
		{
		...
		{			
			...
			// add expected values.
			foreach (ItemValue value in test.Expected)
			{
				AddItem(value, false);
			}
			...
			if (test.Actual != null)
			{
				...
				ResultIDTB.Text += " (" + test.Actual.ResultID.Name.Name + ")";
				...
				for (int ii = 0; ii < test.Actual.Count; ii++)
				{
					...
					{		
						...
						for (int jj = 0; jj < listItem.SubItems.Count; jj++)
						{
							...
							string actual   = Opc.Convert.ToString(GetFieldValue(test.Actual[ii], jj));
	...
	public class TestData
	{
		...
		public Trend Trend;
		public ItemValueCollection Actual;
		public ItemValueCollection Expected;
		public bool Passed;
	}
}
