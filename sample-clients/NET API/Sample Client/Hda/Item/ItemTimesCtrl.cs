...
namespace Opc.Hda.SampleClient
{
	...
	public class ItemTimesCtrl : System.Windows.Forms.UserControl
	{
		...
		/// </summary>
		public void Initialize(Opc.Hda.Server server, ItemTimeCollection times)
		{
			m_server = server;
		...
		/// </summary>
		public ItemTimeCollection GetTimes()
		{
			ItemTimeCollection times = new ItemTimeCollection();
		...
		private void UpdateListItem(ListViewItem listItem, DateTime time)
		{
			...
			{
				listItem.SubItems[ii].Text = Opc.Convert.ToString(GetFieldValue(time, ii));
			}
