...
namespace Opc.Hda.SampleClient
{
	...
	public class ItemListCtrl : System.Windows.Forms.UserControl
	{
		...
		/// </summary>
		public void Initialize(Opc.Hda.Server server, Opc.ItemIdentifier[] items)
		{
		...
		/// </summary>
		public void AddItems(Opc.ItemIdentifier[] items)
		{
			if (items != null)
			{
				foreach (ItemIdentifier item in items)
				{
					AddListItem(new ItemIdentifier(item));
				}
		...
		/// </summary>
		public Opc.ItemIdentifier[] GetItemIDs(bool selected)
		{
		...
		private void AddMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				ItemIdentifier item = new ItemIdentifier();
		...
		private void EditMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (!new ItemEditDlg().ShowDialog((ItemIdentifier)item))
				{
					return;
				}
