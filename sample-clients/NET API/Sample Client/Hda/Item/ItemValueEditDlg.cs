...
{	
	...
	public class ItemValueEditDlg : System.Windows.Forms.Form
	{
		...
		public ItemValueEditDlg()
		{
			...
			foreach (Opc.Da.qualityBits quality in Enum.GetValues(typeof(Opc.Da.qualityBits)))
			{
			...
			foreach (Opc.Da.limitBits limitBits in Enum.GetValues(typeof(Opc.Da.limitBits)))
			{
		...
		/// </summary>
		public ItemValue ShowDialog(ItemValue item)
		{
			...
			Opc.Da.Quality quality = new Opc.Da.Quality();

			quality.QualityBits = (Opc.Da.qualityBits)QualityCB.SelectedItem;
			quality.LimitBits   = (Opc.Da.limitBits)LimitBitsCB.SelectedItem;
