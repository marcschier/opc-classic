...
namespace Opc.Hda.SampleClient
{
	...
	public class ResultListCtrl : System.Windows.Forms.UserControl
	{
		...
		/// </summary>
		public void Initialize(Opc.Hda.Server server, IResult[] results)
		{
		...
		/// </summary>
		public void Initialize(Opc.Hda.Server server, IList[] values, Opc.Hda.ResultCollection[] results)
		{
			if (server == null) throw new ArgumentNullException("server");
			...
			for (int ii = 0; ii < results.Length; ii++)
			{
				for (int jj = 0; jj < results[ii].Count; jj++)
				{
					...
					{
						if (typeof(ItemValue).IsInstanceOfType(values[ii][jj]))
						{
							timestamp = ((ItemValue)values[ii][jj]).Timestamp;
						}
						else if (typeof(AnnotationValue).IsInstanceOfType(values[ii][jj]))
						{
							timestamp = ((AnnotationValue)values[ii][jj]).Timestamp;
						}
					}
		...
		/// </summary>
		public void Initialize(Opc.Hda.Server server, int[] attributeIDs, Opc.Hda.ResultCollection results)
		{
			if (server == null) throw new ArgumentNullException("server");
		...
		public void Initialize(
			...
			ItemTimeCollection         times,
			Opc.Hda.ResultCollection[] results)
		{
			if (server == null) throw new ArgumentNullException("server");
		...
		/// </summary>
		private class ItemTimeResult : ItemIdentifier, IResult
		{
			public ItemTimeResult(ItemIdentifier item, DateTime timestamp, IResult result) : base(item)
			{
				Timestamp      = timestamp;
		...
		/// </summary>
		private class AttributeResult : ItemIdentifier, IResult
		{
			public AttributeResult(ItemIdentifier item, int attributeID, IResult result) : base(item)
			{
		...
		private object GetFieldValue(object item, int fieldID)
		{
			...
			if (typeof(Item).IsInstanceOfType(item))
			{
				...
				{
					return m_server.Aggregates.Find(((Item)item).AggregateID);
				}
			}
			...
			// result.
			if (typeof(IResult).IsInstanceOfType(item))
			{
				if (fieldID == RESULT)
				{
					return ((IResult)item).ResultID; 
				}
			}
		...
		private void ViewMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				{
					new ItemValuesDlg().ShowDialog(m_server, (ItemValueCollection)item, true); 
				}
			}
