...
- `Net10ServerToNativeClientTests` — native-client-launch readiness checks for `external\redist\samples\SampleClient\Da\Simple Client\OpcDaSimpleClient.vcxproj` / `OpcDaSimpleClient.exe`. These soft-skip when the native executable or Windows prerequisites are missing.
