...
- `Net10ServerToNativeClientTests` — native-client-launch readiness checks for `ext\samples\Sample Client\Da\Simple Client\OpcDaSimpleClient.vcxproj` / `OpcDaSimpleClient.exe`. These soft-skip when the native executable or Windows prerequisites are missing.
