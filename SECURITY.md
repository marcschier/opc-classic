...
The project is a pure-managed OPC Classic stack with DCOM and MSRPC components under `src\OpcClassic.Dcom\`. Its security-sensitive surface includes authentication, authorization, channel protection, NDR marshalling, and server callback dispatch.
...
The project intentionally contains in-tree cryptographic code for OPC Classic interoperability. NTLMv2, RC4, and MD4 are hand-rolled in `src\OpcClassic.Dcom\Crypto\` as part of Phase 2E rather than delegated entirely to BCL-backed implementations.
