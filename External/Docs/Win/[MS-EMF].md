...
Restructured record sections according to category; other updates.

Clarified the meaning of the technical content.

Clarified the meaning of the technical content.

Added new sections describing the EMR_COMMENT_EMFPLUS and
...
within one-sixteenth of a pixel. Each point coordinate is a 32-bit value, of which the 28 higher-
order bits are the signed integral part and the 4 lower-order bits are the unsigned fractional
...
The bitmap can be a device-independent bitmap (DIB) section bitmap or it can be a device-
dependent bitmap.
...
Arcs MUST be drawn using the current arc direction, but they MUST NOT reflect the current world-
to-device transform, which might require a rotation along the x-axis or y-axis.
...
current output device. The actual font that is specified by this value is implementation-
dependent.<36>
...
A ColorRef object, which specifies
...
A ColorRef object that specifies
...
The low-order 16-bits is a value
...
The object is a bitmap, vector, or TrueType font, or a Type 1 rasterized font that was
...
it does not match, the system font mapper MAY use a default mechanism to select a back-
up font.<55>
...
external to this EMF record. Any larger file structure in which the font might reside is system-
...
Comment record types  2.3.3

Define formats for specifying arbitrary private data, embedding records in
...
EMR_SETDIBITSTODEVICE  2.3.1.5

Specifies a block transfer of pixels from specified scanlines of a source
...
The pixels in the source bitmap do not specify alpha transparency. In this case, the
...
XformSrc (24 bytes): An XForm object (section 2.2.28) that specifies a world-space to page-

space transform to apply to the source bitmap.
...
The upper-left corner of the source rectangle is mapped to the first point in this array, the upper-
right corner to the second point, and the lower-left corner to the third point. The lower-right
...
If the signs of the source and destination height and width fields differ, this record specifies a mirror-
image copy of the source bitmap to the destination. That is, if cxSrc and cxDest have different signs,
...
Specifies a clipping region that consists of the current clipping region
...
EMR_INTERSECTCLIPRECT  2.3.2.3

Specifies a clipping region from the intersection of the current clipping
...
Specifies a clipping region as the current clipping region moved by a
specified offset.
...
Specifies a clipping region as the current clipping region combined with
...
Indicates the start of the metafile and specifies properties of the device on which the
...
Draws an elliptical arc, resetting the current drawing position to the
...
Draws a chord, which is a region bounded by the intersection of an
...
Box (16 bytes): A RectL object ([MS-WMF] section 2.2.2.19), which specifies the inclusive-

inclusive bounding rectangle in logical units.
...
Bounds (16 bytes): A RectL object ([MS-WMF] section 2.2.2.19), which specifies the inclusive-
...
starting point.
 ...
 Line width   Device supports wideline
...
Passes arbitrary information to the printer driver. The intent is that the
information results in drawing being done.
...
Defines a logical brush with a LogBrushEx object (section
2.2.12).
...
Defines a logical color space with a LogColorSpace object
...
Defines a logical color space with a LogColorSpaceW object
...
EMR_CREATEDIBPATTERNBRUSHPT  2.3.7.4

Defines a pattern brush with a DeviceIndependentBitmap object
...
Defines a monochrome pattern brush with a monochrome
...
Defines a logical palette with a LogPalette object (section
2.2.17).
...
EMR_COLORCORRECTPALETTE  2.3.8.1

Specifies how to correct the entries of a LogPalette
...
Specifies how to delete a logical color space from the EMF object
...
This record closes path bracket construction and selects the path into the playback device
...
This record redefines the current path as the area that would be painted if its path were
...
EMR_COLORMATCHTOTARGETW  2.3.11.1

Specifies how to preview colors as they would appear on the
target device.
...
Forces the font mapper to match fonts based on their
...
This record maps palette entries from the current LogPalette
...
Saves the current state of the playback device context in an array
...
The EMR_SETWORLDTRANSFORM record specifies a transform for the current world-space to page-
space transform in the playback device context.
...
transform in logical units. This transform defines a new value for the current world-space to page-
space transform.
...
ColorAdjustment (section 2.2.2)

EMR_SETCOLORADJUSTMENT (section 2.3.11.13)

ColorProfile

UINT8[]

EMR_COLORMATCHTOTARGETW (section 2.3.11.1)
...
EMR_SETICMMODE (section 2.3.11.14)

PixelFormat

PixelFormatDescriptor (section
2.2.22)
...
EMR_SETMAPPERFLAGS (section 2.3.11.20)

ForceUFIMapping

UniversalFontId (section 2.2.27)

EMR_FORCEUFIMAPPING (section 2.3.11.2)

LinkedUFIs

UniversalFontId[]

EMR_SETLINKEDUFIS (section 2.3.11.18)

TextAlignment

UINT32

EMR_SETTEXTALIGN (section 2.3.11.25)

TextJustification

UINT32 [2]

EMR_SETTEXTJUSTIFICATION (section 2.3.11.27)
...
ColorRef ([MS-WMF] section 2.2.2.8)
...
CharSet (1 byte): 0x01 specifies the default character set, from the CharacterSet enumeration ([MS-
WMF] section 2.1.1.5).
...
Area of the window passed to GetDC
...
(dmPelsWidth X dmPelsHeight) of the DEVMODE
