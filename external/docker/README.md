...
```pwsh
docker compose --file external/docker/docker-compose.test.yml --profile interactive build
```
...
Builds and runs the hand-rolled native OPC DA smoke client (`opc-test.exe`) from `external\docker\opc-c-client\build\opc-test.cpp`. It resolves a ProgID on a target host, calls `AddGroup`, `AddItems`, `Read`, then removes the group.
...
enumeration output. See `external\docker\opc-testclient\README.md`.
...
`external/docker/run-matrix.ps1` creates the network idempotently with:
