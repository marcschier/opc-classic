...
Extracted from `ext/Spec/historical-dataaccess-1.20-errata.zip` (`Historical DataAccess 1.20 Errata.docx`). The errata is table-heavy/free-form, so the converted text is retained here verbatim and referenced from the specification preamble instead of applying unreliable in-place replacements.
