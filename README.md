...
On Windows, also add `Opc.Classic.Windows` to host a server through DCOM. The full granular per-spec packages (`Opc.Classic.Da`, `Opc.Classic.Ae`, …) remain available on the GitHub Packages feed — see [docs/RELEASE_PROCESS.md](docs/RELEASE_PROCESS.md).
