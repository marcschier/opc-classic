...
- **Cross-platform.** Talk to and host OPC DA / AE / HDA servers from Linux and macOS, not just Windows. No `[ComImport]`, no `ole32.dll` P/Invoke, no Windows-specific runtime — a pure-managed DCOM stack (ex-SharpInterop, derived from j-Interop) is the foundation.
- **NativeAOT-compatible libraries.** Consumer apps can publish with `dotnet publish -p:PublishAot=true` against any `OpcClassic.*` package and see zero `IL2xxx` / `IL3xxx` warnings. All in-tree code is source-generated or hand-rolled — no reflection emit, no expression-tree compile, no `MethodInfo.Invoke`.
...
| `samples/` | Quickstart sample apps and the AOT canary (`samples/OpcClassic.AotCanary`). |
...
```bash
dotnet publish samples/OpcClassic.AotCanary -c Release -p:PublishAot=true
```
...
A pure-managed MSRPC/DCOM stack (`OpcClassic.Dcom`) is the foundation. On top of it sit per-spec assemblies (`OpcClassic.Da`, `OpcClassic.Ae`, `OpcClassic.Hda`, …) that translate the OPC Classic interface semantics into managed APIs. A Roslyn source generator (`OpcClassic.Generators`) emits AOT-safe call shims, NDR marshallers, and `LocalCoClass` dispatch tables — replacing every reflection / expression-tree code path that would otherwise break NativeAOT.

See [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) (coming as part of Phase 15A).

## License

[EPL-1.0](LICENSE) (current; final disposition pending Phase 1E). Per-file `SPDX-License-Identifier` headers are being added as code is migrated.
