...
```pwsh
docker build --file docker/opc-c-server/Dockerfile --tag opc-classic/c-server .
docker run --rm --network opc-test-net opc-classic/c-server
```

The image compiles `docker\opc-c-server\build\opc-sample-server.cpp` against headers from `ext\inc`, registers `OPC.SampleServer.1`, and exposes the DCOM endpoint mapper plus the pinned dynamic range `49152-49200`.
