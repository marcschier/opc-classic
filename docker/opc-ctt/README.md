...
```pwsh
# (Bring the fleet up first; see docker/docker-compose.test.yml)
...
- `docker/opc-ctt/Dockerfile` — image definition
- `docker/opc-ctt/dcom-test-acls.reg` — relaxed DCOM ACL imports
- `docker/opc-ctt/run-ctt.ps1` — ENTRYPOINT shim
- `docker/docker-compose.test.yml` — orchestrates the full test fleet
- `docker/README.md` — top-level fleet documentation
