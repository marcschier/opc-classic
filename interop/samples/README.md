...
- `SampleServer/Da/` — DA sample server projects:
...
- `SampleServer/Ae/` — AE Simple Server (`OPCSample.OPCEventServer.1`)
- `SampleServer/Hda/` — HDA Simple Server (`OPCSample.OpcHdaServer.1`)
...
set SOLUTIONDIR=%CD%\interop\samples\
msbuild "interop\samples\SampleServer\Da\Server\OpcDaServer.vcxproj" /p:Configuration=Release /p:Platform=Win32 /p:SolutionDir="%SOLUTIONDIR%" /p:PlatformToolset=v143 /p:WindowsTargetPlatformVersion=10.0 /m
msbuild "interop\samples\SampleServer\Ae\OpcAeServer.vcxproj" /p:Configuration=Release /p:Platform=Win32 /p:SolutionDir="%SOLUTIONDIR%" /p:PlatformToolset=v143 /p:WindowsTargetPlatformVersion=10.0 /m
msbuild "interop\samples\SampleServer\Hda\Server\OpcHdaServer.vcxproj" /p:Configuration=Release /p:Platform=Win32 /p:SolutionDir="%SOLUTIONDIR%" /p:PlatformToolset=v143 /p:WindowsTargetPlatformVersion=10.0 /m
```

The projects emit EXEs under:

```text
interop\samples\BuildOutput\bin\servers\Win32\Release\
```
...
```cmd
interop\samples\BuildOutput\bin\servers\Win32\Release\OpcDaServer.exe /RegServer
interop\samples\BuildOutput\bin\servers\Win32\Release\OpcAeServer.exe /RegServer
interop\samples\BuildOutput\bin\servers\Win32\Release\OpcHdaServer.exe /RegServer
```
...
`SampleServer/Ae/OPCEventServer.rgs` and `OpcAeServer.cpp`.
