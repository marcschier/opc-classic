...
.DESCRIPTION
    ...
    interop/docs/testserver-registration-spec.md for the full audit.
...
.PARAMETER ExePath
    ...
    interop\build\x64\Release\, the vendored CMake output
    produced by interop\tools\build-testserver.ps1). The sibling proxy/stub
...
.EXAMPLE
    .\interop\tools\register-testserver.ps1                  # register
    .\interop\tools\register-testserver.ps1 -Unregister      # remove TestServer entries and copied DLLs
#>
...
function Resolve-DefaultTestServerPath {
    ...
    $candidate = Join-Path $repoRoot 'interop\build\x64\Release\OpcTestServer_x64.exe'
...
function Install-ProxyStubDlls {
    ...
    # interop/docs/testserver-registration-spec.md
    $dllNames = @(
        'opccomn_ps.dll',
        'opcproxy.dll',
        'opc_aeps.dll',
    ...
    foreach ($dllName in $dllNames) {
        ...
        if (-not (Test-Path -LiteralPath $source)) {
            Write-Warning "Skipping $dllName; not present in $ArtifactDirectory. Re-run interop\tools\build-testserver.ps1 to produce the full proxy/stub set; DA-only marshalling needs opccomn_ps + opcproxy at a minimum."
            continue
        }
...
function Copy-TestServerConfig {
    ...
    # at interop/CMakeLists.txt); a plain `cmake --build` step
    # leaves the source file at interop/samples/OpcTestServer/
    ...
    foreach ($candidate in $candidates) {
        if (Test-Path -LiteralPath $candidate) {
            ...
            # See interop/docs/testserver-registration-spec.md for the full
            # diagnostic trace.
...
elseif (-not $Unregister) {
    Write-Error 'Cannot find OpcTestServer_x64.exe. Run interop\tools\build-testserver.ps1 first, or pass -ExePath explicitly.'
    exit 1
}
else {
