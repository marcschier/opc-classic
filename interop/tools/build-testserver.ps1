...
.DESCRIPTION
    ...
    an external clone. Produces (under interop/build/x64/Release/):
      ...
      see interop\docs\testserver-registration-spec.md)
...
.EXAMPLE
    ...
    .\interop\tools\build-testserver.ps1 -Clean
#>
...
try {
    ...
    # Build the full proxy/stub set so the interop\tools\register-testserver.ps1 script
    # has all 8 DLLs available to register (matches what the CoreComponents
    # install rules deploy; see interop\docs\testserver-registration-spec.md).
    ...
    if (Test-Path $exe) {
        ...
        Write-Host "Next: register with 'interop\tools\register-testserver.ps1' (elevated)."
    } else {
