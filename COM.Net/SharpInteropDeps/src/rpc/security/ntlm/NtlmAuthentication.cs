    ...
    public class NtlmAuthentication {
        ...
        public NtlmAuthentication(Properties properties) {
            ...
            if (properties != null) {
                ...
                user = (string)properties.GetProperty(rpc.Security.USERNAME);
                password = (string)properties.GetProperty(rpc.Security.PASSWORD);
            }
        ...
        private int AdjustFlags(int flags) {
            if (_unicodeSupported && ((flags & NtlmFlags.NtlmsspNegotiateUnicode) != 0)) {
