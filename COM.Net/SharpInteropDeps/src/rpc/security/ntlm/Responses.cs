...
//ORIGINAL LINE: public static byte[] getLMv2Response(String target, String user, String password, byte[] challenge, byte[] clientNonce) throws Exception
        ...
        public static sbyte[] getNTLM2SessionResponse(string password, sbyte[] challenge, sbyte[] clientNonce)
		{
			...
			var sessionHash = new sbyte[8];
			Array.Copy(md5.digest(), 0, sessionHash, 0, 8);
			return lmResponse(ntlmHash_Renamed, sessionHash);
		}
...
//ORIGINAL LINE: private static byte[] lmHash(String password) throws Exception
		private static sbyte[] lmHash(string password)
		{
			...
			var lmHash_Renamed = new sbyte[16];
...
//ORIGINAL LINE: static byte[] ntlmv2Hash(String target, String user, String password) throws Exception
        ...
        /// <exception cref="InvalidOperationException">  </exception>
        //JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
        //ORIGINAL LINE: private static byte[] lmResponse(byte[] hash, byte[] challenge) throws java.security.NoSuchAlgorithmException, javax.crypto.NoSuchPaddingException, java.security.InvalidKeyException, System.InvalidOperationException, javax.crypto.IllegalBlockSizeException, javax.crypto.BadPaddingException
        private static sbyte[] lmResponse(sbyte[] hash, sbyte[] challenge)
		{
			var keyBytes = new sbyte[21];
			...
			var lmResponse_Renamed = new sbyte[24];
...
//ORIGINAL LINE: private static byte[] lmv2Response(byte[] hash, byte[] clientData, byte[] challenge) throws Exception
		...
		{
			var data = new sbyte[challenge.Length + clientData.Length];
			...
			var lmv2Response_Renamed = new sbyte[mac.Length + clientData.Length];
		...
		{
			sbyte[] blobSignature = {(sbyte) 0x01, (sbyte) 0x01, (sbyte) 0x00, (sbyte) 0x00};
			sbyte[] reserved = {(sbyte) 0x00, (sbyte) 0x00, (sbyte) 0x00, (sbyte) 0x00};
			sbyte[] unknown1 = {(sbyte) 0x00, (sbyte) 0x00, (sbyte) 0x00, (sbyte) 0x00};
			sbyte[] unknown2 = {(sbyte) 0x00, (sbyte) 0x00, (sbyte) 0x00, (sbyte) 0x00};
			var time = DateTimeHelperClass.CurrentUnixTimeMillis();
			time += 11644473600000l; // milliseconds from January 1, 1601 -> epoch.
			time *= 10000; // tenths of a microsecond.
			// convert to little-endian byte array.
			var timestamp = new sbyte[8];
			for (var i = 0; i < 8; i++)
			{
				timestamp[i] = (sbyte) time;
				time = (long)((ulong)time >> 8);
			}
			var blob = new sbyte[blobSignature.Length + reserved.Length + timestamp.Length + clientNonce.Length + unknown1.Length + targetInformation.Length + unknown2.Length];
			var offset = 0;
...
//ORIGINAL LINE: static byte[] hmacMD5(byte[] data, byte[] key) throws java.security.NoSuchAlgorithmException
		...
		{
			var ipad = new sbyte[64];
			var opad = new sbyte[64];
			for (var i = 0; i < 64; i++)
			{
				ipad[i] = (sbyte) 0x36;
				opad[i] = (sbyte) 0x5c;
			...
			var content = new sbyte[data.Length + 64];
		...
		{
			var keyBytes = new sbyte[7];
			Array.Copy(bytes, offset, keyBytes, 0, 7);
			var material = new sbyte[8];
		...
		private static void oddParity(sbyte[] bytes)
		{
			...
			{
				var b = bytes[i];
				var needsParity = ((((int)((uint)b >> 7)) ^ ((int)((uint)b >> 6)) ^ ((int)((uint)b >> 5)) ^ ((int)((uint)b >> 4)) ^ ((int)((uint)b >> 3)) ^ ((int)((uint)b >> 2)) ^ ((int)((uint)b >> 1))) & 0x01) == 0;
				if (needsParity)
				{
					bytes[i] |= (sbyte) 0x01;
				}
