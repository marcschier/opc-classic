...
namespace org.jinterop.dcom.test {
	...
	using IJIUnsigned = org.jinterop.dcom.core.IJIUnsigned;
...
//JAVA TO C# CONVERTER WARNING: The .NET Type.FullName property will not always yield results identical to the Java Class.getName method:
						 ...
						 int i = name2.LastIndexOf("L", StringComparison.Ordinal);
...
//JAVA TO C# CONVERTER WARNING: The .NET Type.FullName property will not always yield results identical to the Java Class.getName method:
				  ...
				  JIArray arrrry = new JIArray(new long?[][] {
...
//JAVA TO C# CONVERTER WARNING: The .NET Type.FullName property will not always yield results identical to the Java Class.getName method:
					string name = oi.GetType().FullName;
					object subArray = oi;
					int dimension = 0;
					...
					int[] upperBounds = new int[upperBounds2.Count];
	...
	//			 Object newArray = createArray(oi, Class.forName(clazzName), dimension);
					object newArray = CreateArray(new long?[][] {
						new long?[] { new long?(1),new long?(2) },
						new long?[] { new long?(3), new long?(4), new long?(5) },
						new long?[] { new long?(3), new long?(4), new long?(5) }
					},
...
//ORIGINAL LINE: int[][] g1 = new int[10][10];
				  int[][] g1 = RectangularArrays.ReturnRectangularIntArray(10, 10);
				  int?[][] g = new int?[10][10];
				 ...
				 // System.out.println(rrr.getClass().getSimpleName());
				  ...
				  Constructor cc = typeof(JIVariant).GetConstructor(new Type[]{ typeof(int), typeof(bool) });
				  JIVariant vv = (JIVariant)cc.newInstance(new object[]{ y, Convert.ToBoolean(true) });
				  object o007 = new int?(1);
				  Driver.Cc(o007);
				  ...
				  sbyte?[][] bgh = new sbyte?[10][0];
				  long jj = 2147483670L;
				  short b = unchecked((short)40000);
				  sbyte b1123 = (sbyte)b;
				  string msd = null + "";
				  UID uid = new UID();
...
//ORIGINAL LINE: object[][] obj = new object[6][2];
				  object[][] obj = RectangularArrays.ReturnRectangularObjectArray(6, 2);
				  ...
				  int toSend = unchecked((int)0xFFFFFFFF);
				  Console.WriteLine(toSend.ToString("x"));
				  double gss = 100020 % 10000;
				  string toSend2 = (toSend.ToString("x"));
				  sbyte[] hibuffer = new sbyte[]{ 0,0,0,0,0,0,0,0 };
				  sbyte[] lowbuffer = new sbyte[]{ 0,0,0,0,0,0,0,0 };
				  string lo = "";
				  if (toSend2.Length > 8) {
	...
	//			  long va = d & 0xF;
				  ...
				  sbyte[] buffer = new sbyte[1148]; //1144
	...
	//				computeLengthArray(f);
					  int?[] yx = new int?[]{ new int?(19),new int?(20),new int?(22),new int?(23) };
	...
	//				serializeArray(yx);
	//
					  object array = DeSerializeArray(typeof(float?),new int[]{ 10,20 },2);
	...
	//			  byte[] array = address.getAddress();
					  sbyte[] b1 = new sbyte[10]; //{(byte)0x9c,(byte)0x3f,(byte)0x16,(byte)0};
...
//JAVA TO C# CONVERTER WARNING: The .NET Type.FullName property will not always yield results identical to the Java Class.getName method:
		...
		internal static object DeSerializeArray(Type arrayType, int[] upperBounds, int dimension) {
			...
			for (int i = 0; i < upperBounds[upperBounds.Length - dimension] ; i++) {
				if (dimension == 1) {
					//fill value here
					((System.Array)array).SetValue(new float?(i), i);
				}
				else {
