...
namespace org.jinterop.dcom.test {
	...
	using JICurrency = org.jinterop.dcom.core.JICurrency;
...
//ORIGINAL LINE: public FirstContact_Stub(String address) throws Exception
		public FirstContact_Stub(string address) {
			JIArray arry123 = new JIArray(new sbyte?[10][0],true);
			JIStruct struct123 = new JIStruct();
			struct123.AddMember(arry123);
			struct123.AddMember(0,new int?(1));
	...
	//
				...
				ResourceBundle bundle = JISystem.ErrorMessages;
	...
	//            JIVariant[] rt = dispatch.callMethodA("testSafeArrayOfVariants", new Object[]{JIVariant.EMPTY()_BYREF});
			   ...
			   // dispatch.callMethod("testArrayOfVariants",new Object[]{new JIArray(new JIVariant[]{new JIVariant(new JIArray(new JIString[]{new JIString("ab"),new JIString("cd")}))},true)});
				...
				object[] ret0 = dispatch.CallMethodA("LongArray", new object[]{ new JIVariant(new JIArray(new int?[]{ new int?(1),new int?(2),new int?(4) },true), true) });
				JIArray ret01 = ((JIVariant)ret0[1]).ObjectAsArray;
				ret0 = dispatch.CallMethodA("ReadAsicRegisterBlock", new object[]{ new JIString("Chonap"),new JIString("Cho"),new JIVariant(new JIArray(new JIUnsignedShort[]{ (JIUnsignedShort)JIUnsignedFactory.GetUnsigned(new int?(4000), JIFlags.FLAG_REPRESENTATION_UNSIGNED_SHORT),(JIUnsignedShort)JIUnsignedFactory.GetUnsigned(new int?(4001), JIFlags.FLAG_REPRESENTATION_UNSIGNED_SHORT) },true),true),new JIVariant(new JIArray(new JIUnsignedInteger[]{ (JIUnsignedInteger)JIUnsignedFactory.GetUnsigned(new long?(9999), JIFlags.FLAG_REPRESENTATION_UNSIGNED_INT),(JIUnsignedInteger)JIUnsignedFactory.GetUnsigned(new long?(9999), JIFlags.FLAG_REPRESENTATION_UNSIGNED_INT) },true),true), false,true });
				ret01 = ((JIVariant)ret0[1]).ObjectAsArray;

				ret0 = dispatch.CallMethodA("testSA1", new object[]{ new JIVariant(new JIArray(new bool?[]{ false,true },true),true),new JIVariant(new JIArray(new float?[]{ new float?(123.4),new float?(123.4) },true),true),new JIVariant(new JIArray(new double?[]{ new double?(123.4),new double?(123.4) },true),true) });
				ret01 = ((JIVariant)ret0[1]).ObjectAsArray;
				...
				ret01 = ((JIVariant)ret0[1]).ObjectAsArray;
				ret0 = dispatch.CallMethodA("testSA2", new object[]{ new JIVariant(new JIArray(new sbyte?[]{ new sbyte?((sbyte)1),new sbyte?((sbyte)1) },true),true), new JIVariant(new JIArray(new JIVariant[]{ new JIVariant(dispatch,true) },true),true),new JIVariant(new JIArray(new JIVariant[]{ new JIVariant(unknown,true) },true),true) });
				ret01 = ((JIVariant)ret0[1]).ObjectAsArray;
				ret01 = ((JIVariant)ret0[2]).ObjectAsArray;
				ret01 = ((JIVariant)ret0[3]).ObjectAsArray;
	...
	//		 callObject.addInParamAsVariant(vExc,JIFlags.FLAG_NULL);
				...
				JIArray arrt = ((JIVariant)t[0]).ObjectAsVariant.ObjectAsArray;
	...
	//			dispatch.put("TestProperty1", new Object[]{new Short((short)1), new JIString("Hello")});
		//		dispatch.put("TestProperty2", new Object[]{new Short((short)1), new Short((short)2), new Integer(3)});
				...
				int type = typeLib.TypeInfoCount;
				typeLib.LibAttr;
	...
	//			result = handle.call(obj);
				...
				obj.ReInit();
				obj.Opnum = 49;
				obj.AddInParamAsPointer(new JIPointer(new JIPointer(new int?(100))),JIFlags.FLAG_NULL);
				handle.Call(obj);

				obj.ReInit();
				obj.Opnum = 53;
				obj.AddInParamAsPointer(new JIPointer((new int?(100))),JIFlags.FLAG_NULL);
				handle.Call(obj);
	...
	//			 ret = dispatch.callMethodA("testBSTR02",new Object[]{new JIVariant(new JIString("Hello"),true)});
		...
		//		ret = dispatch.callMethodA("testBSTR03",new Object[]{new JIVariant(new JIString("Hello"),true)});
				...
				JIArray array2 = variant3[1].ObjectAsArray;

				 object[][] newValue = new object[][] {
					 new object[] { new JIString("defe"), new bool?(false), new double?(98765.0 / 12345.0) },
					 new object[] { DateTime.Now, new int?(5454), new float?(22.0 / 7.0) },
					 new object[] { new bool?(true), new JIString("dffe"), DateTime.Now }
				 };
				...
				//variant2[1].getObjectAsArray();
				 ...
				 variant2 = dispatch.CallMethodA("testSAFEARRAY01",new object[]{ new JIVariant(new JIArray(new int?[]{ new int?(100),new int?(100),new int?(200) },true),true) });
				...
				JIArray array3 = variant4[1].ObjectAsArray;

				variant = dispatch.CallMethodA("testSA",new object[]{ new JIVariant(new DateTime?(DateTimeHelperClass.CurrentUnixTimeMillis()),true) });
				variant2 = dispatch.CallMethodA("testSAFEARRAY01",new object[]{ new JIVariant(new JIArray(new int?[]{ new int?(100),new int?(100),new int?(200) })) });
				//variant2[1].getObjectAsArray();
			...
			//	dispatch.callMethod(3,new Object[]{new JIVariant(new JICurrency(10,0))});
				...
				JILocalCoClass component = new JILocalCoClass(interfaceDefinition,typeof(Test));
