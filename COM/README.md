# COM/ — OPC Foundation Sample Servers (Native C++ Reference)
...
- `Sample Client/Da/Simple Client/` — DA test client (for Phase 14D-B compatibility)
- `Include/` — OPC IDL headers and proxy/stub artifacts (`opcda.h`, `opc_ae.h`, `opchda.h`, `opcproxy.dll`, etc.)
- `Shared/Utils/` — common C++ helpers used by the native sample projects

No legacy `.sln` files are currently preserved under `COM/`; the native
...
- Windows SDK 10
- OPC Foundation Core Components installed separately (provides `opcproxy.dll` for marshalling and OpcEnum)
...
set SOLUTIONDIR=%CD%\COM\
msbuild "COM\Sample Server\Da\Server\OpcDaServer.vcxproj" /p:Configuration=Release /p:Platform=Win32 /p:SolutionDir="%SOLUTIONDIR%" /p:PlatformToolset=v143 /p:WindowsTargetPlatformVersion=10.0 /m
msbuild "COM\Sample Server\Ae\OpcAeServer.vcxproj" /p:Configuration=Release /p:Platform=Win32 /p:SolutionDir="%SOLUTIONDIR%" /p:PlatformToolset=v143 /p:WindowsTargetPlatformVersion=10.0 /m
msbuild "COM\Sample Server\Hda\Server\OpcHdaServer.vcxproj" /p:Configuration=Release /p:Platform=Win32 /p:SolutionDir="%SOLUTIONDIR%" /p:PlatformToolset=v143 /p:WindowsTargetPlatformVersion=10.0 /m
```

The projects emit EXEs under:

```text
COM\BuildOutput\bin\servers\Win32\Release\
```
...
```cmd
COM\BuildOutput\bin\servers\Win32\Release\OpcDaServer.exe /RegServer
COM\BuildOutput\bin\servers\Win32\Release\OpcAeServer.exe /RegServer
COM\BuildOutput\bin\servers\Win32\Release\OpcHdaServer.exe /RegServer
```
...
DA and HDA derive their ProgIDs in `COM/Shared/Utils/COpcClassFactory.cpp`
