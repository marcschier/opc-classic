...
{	
	...
	/// </summary>
	public delegate void BrowseFiltersChangedCallback(int maxElements, BrowseFilter[] filters);
	...
	public class BrowseFiltersDlg : System.Windows.Forms.Form
	{
			...
			BrowseFiltersChangedCallback callback)
		{
			...
			if (browser != null)
			{
				foreach (BrowseFilter filter in browser.Filters)
				{
					switch (filter.AttributeID)
					{
						case AttributeID.ITEMID:
						{
		...
		/// </summary>
		private void AddFilter(BrowseFilter filter)
		{
			ListViewItem item = new ListViewItem("", Resources.IMAGE_ITEM_PROPERTY);
			...
			item.SubItems[2].Text = Opc.Convert.ToString(filter.FilterValue);
		...
		private void ApplyChanges()
		{
			...
			if (ItemNameTB.Text != "")
			{
				...
				filter.AttributeID = AttributeID.ITEMID;
			...
			if (DataTypeCTRL.SelectedType != null && DataTypeCTRL.SelectedType != typeof(object))
			{
				...
				filter.AttributeID = AttributeID.DATA_TYPE;
				filter.Operator    = Operator.Equal;
			...
			if (m_callback != null)
			{
				m_callback((int)MaxElementsCTRL.Value, (BrowseFilter[])filters.ToArray(typeof(BrowseFilter)));
			}
		}
		...
		private void AddMI_Click(object sender, System.EventArgs e)
		{
			...
			excludeIDs.Add(AttributeID.ITEMID);
			excludeIDs.Add(AttributeID.DATA_TYPE);

			foreach (ListViewItem item in BrowseFiltersLV.Items)
			{				
				excludeIDs.Add(((BrowseFilter)item.Tag).AttributeID);
			}

			// edit filter values.
			BrowseFilter filter = new AttributeFilterEditDlg().ShowDialog(m_server, excludeIDs);
