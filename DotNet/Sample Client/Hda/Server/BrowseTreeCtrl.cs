...
namespace Opc.Hda.SampleClient
{
	...
	public class BrowseTreeCtrl : System.Windows.Forms.UserControl
	{
		...
		/// </summary>
		public delegate void ItemPickedEventHandler(ItemIdentifier[] items);
		...
		/// </summary>
		public delegate void ItemSelectedEventHandler(ItemIdentifier item);
		...
		/// </summary>
		public void Browse(Opc.Hda.Server server, BrowseFilter[] filters)
		{
		...
		/// </summary>
		private Opc.Hda.IBrowser m_browser = null;
		...
		/// </summary>
		private void Browse(TreeNode parent, BrowseElement branch)
		{
		...
		private void BrowseTV_MouseDown(object sender, System.Windows.Forms.MouseEventArgs e)
		{
			...
			{
				BrowseElement element = (BrowseElement)clickedNode.Tag;
		...
		/// </summary>
		private void OnBrowseFiltersChanged(int maxElements, BrowseFilter[] filters)
		{
			try
			{
				...
				if (node != null)
				{
					BrowseElement branch = null;

					if (node.Tag != null && node.Tag.GetType() == typeof(BrowseElement))
					{
						branch = (BrowseElement)node.Tag;
					}
		...
		private void PickMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (node == null || node.Tag == null || node.Tag.GetType() != typeof(BrowseElement))
				{
					return;
				}
				...
				if (element.IsItem)
				{
					if (ItemPicked != null)
					{
						ItemPicked(new ItemIdentifier[] { element });
					}
				}
			}
		...
		private void PickChildrenMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				{
					if (typeof(BrowseElement).IsInstanceOfType(child.Tag))
					{
						BrowseElement element = (BrowseElement)child.Tag;
				...
				if (items.Count > 0)
				{
					if (ItemPicked != null)
					{
						ItemPicked((ItemIdentifier[])items.ToArray(typeof(ItemIdentifier)));
					}
				}
			}
			catch (Exception exception)
		...
		private void BrowseTV_DoubleClick(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (node == null || !typeof(BrowseElement).IsInstanceOfType(node.Tag))
				{
					return;
				}
		...
		private void BrowseTV_AfterSelect(object sender, System.Windows.Forms.TreeViewEventArgs e)
		{
			try
			{
				...
				if (node != null && typeof(BrowseElement).IsInstanceOfType(node.Tag))
				{
					if (((BrowseElement)node.Tag).IsItem)
					{
