...
namespace Opc.Hda.SampleClient
{
	...
	public class DeleteValuesDlg : System.Windows.Forms.Form
	{
		...
		private Opc.Hda.SampleClient.ResultListCtrl ResultsCTRL;
		...
		private void InitializeComponent()
		{
			...
			this.ResultsCTRL = new Opc.Hda.SampleClient.ResultListCtrl();
		...
		/// </summary>
		public ResultCollection[] ShowDialog(Opc.Hda.Server server, RequestType type, bool synchronous)
		{
		...
		/// </summary>
		public ResultCollection[] ShowDialog(Opc.Hda.Trend trend, RequestType type, bool synchronous)
		{
		...
		/// </summary>
		private ItemIdentifier[] m_items = null;

		/// <summary>
		/// The set of new items which will be added to the trend.
		/// </summary>
		private ResultCollection[] m_results = null;
		...
		/// </summary>
		private ResultCollection[] SyncDelete(Item[] items)
		{
			...
			switch (m_type)
			{
				...
				{
					IdentifiedResult[] results = m_trend.Delete(items);

					if (results != null)
					{
						ResultCollection[] collections = new ResultCollection[results.Length];

						for (int ii = 0; ii < results.Length; ii++)
						{
							collections[ii] = new ResultCollection(results[ii]);
							collections[ii].Add(new Result(results[ii]));
						}
		...
		/// </summary>
		private IdentifiedResult[] AsyncDelete(Item[] items)
		{
			...
			switch (m_type)
			{
				...
				case RequestType.DeleteRaw:
				{
					results = m_trend.Delete(
						items,
						null,
						new UpdateCompleteEventHandler(OnDeleteComplete),
						out m_request);
		...
		private void DoItemDelete()
		{
			...
			{
				// delete data.
				ResultCollection[] results = SyncDelete(new Item[] { item });
			...
			else
			{
				...
				// begin delete data.
				IdentifiedResult[] results = AsyncDelete(new Item[] { item });
		...
		private void DoTrendDelete()
		{
			...
			{
				// delete data.
				ResultCollection[] results = SyncDelete(items);
			...
			else
			{
				...
				// begin delete data.
				IdentifiedResult[] results = AsyncDelete(items);
		...
		/// </summary>
		public void OnDeleteComplete(IRequest request, ResultCollection[] results)
		{
			...
			if (InvokeRequired)
			{
				BeginInvoke(new UpdateCompleteEventHandler(OnDeleteComplete), new object[] { request, results });
				return;
			}
		...
		{
			if (typeof(ResultCollection).IsInstanceOfType(result))
			{
				if (m_type == RequestType.DeleteAtTime)
				{
					ResultsCTRL.Initialize(m_server, m_trend.Timestamps, new ResultCollection[] { (ResultCollection)result });
				}
				else
				{
					ResultsCTRL.Initialize(m_server, (ItemTimeCollection)null, new ResultCollection[] { (ResultCollection)result });
				}
			}
		}
	}
