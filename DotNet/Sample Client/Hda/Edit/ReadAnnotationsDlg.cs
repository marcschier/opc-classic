...
namespace Opc.Hda.SampleClient
{
	...
	public class ReadAnnotationsDlg : System.Windows.Forms.Form
	{
		...
		private Opc.Hda.SampleClient.AnnotationValuesCtrl ResultsCTRL;
		...
		private void InitializeComponent()
		{
			...
			this.ResultsCTRL = new Opc.Hda.SampleClient.AnnotationValuesCtrl();
		...
		/// </summary>
		public AnnotationValueCollection ShowDialog(Opc.Hda.Server server, bool synchronous)
		{
		...
		/// </summary>
		public AnnotationValueCollection[] ShowDialog(Opc.Hda.Trend trend, bool synchronous)
		{
		...
		/// </summary>
		private AnnotationValueCollection[] m_results = null;
		...
		private void DoItemRead()
		{
			...
			{
				// read data.
				AnnotationValueCollection[] results = m_trend.ReadAnnotations(new Item[] { item });
			...
			else
			{
				...
				// begin read data.
				IdentifiedResult[] results = m_trend.ReadAnnotations(
					new Item[] { item },
					null,
					new ReadAnnotationsEventHandler(OnReadComplete),
					out m_request);
		...
		{			
			...
			{
				// read data.
				AnnotationValueCollection[] results = m_trend.ReadAnnotations(items);
		...
		/// </summary>
		public void OnReadComplete(IRequest request, AnnotationValueCollection[] results)
		{
			...
			if (InvokeRequired)
			{
				BeginInvoke(new ReadAnnotationsEventHandler(OnReadComplete), new object[] { request, results });
				return;
			}
		...
		{
			if (typeof(AnnotationValueCollection).IsInstanceOfType(result))
			{
				ResultsCTRL.Initialize(m_server, (AnnotationValueCollection)result);
			}
		}
	}
}
