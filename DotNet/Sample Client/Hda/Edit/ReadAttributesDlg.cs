...
namespace Opc.Hda.SampleClient
{
	...
	public class ReadAttributesDlg : System.Windows.Forms.Form
	{
		...
		private Opc.Hda.SampleClient.AttributesViewCtrl ResultsCTRL;
		private Opc.Hda.SampleClient.AttributesSelectCtrl AttributesCTRL;
		...
		private void InitializeComponent()
		{
			...
			this.AttributesCTRL = new Opc.Hda.SampleClient.AttributesSelectCtrl();
			this.ResultsCTRL = new Opc.Hda.SampleClient.AttributesViewCtrl();
		...
		/// </summary>
		public ItemAttributeCollection ShowDialog(Opc.Hda.Server server, bool synchronous)
		{
			return ShowDialog(server, null, new Time("NOW"), null, synchronous);
		}

		/// <summary>
		/// Prompts the user to select attributes to read for an item.
		/// </summary>
		public ItemAttributeCollection ShowDialog(
		...
		/// </summary>
		private ItemAttributeCollection m_results = null;
		...
		private void DoItemRead()
		{
			...
			{
				// read data.
				ItemAttributeCollection results = m_trend.ReadAttributes(item, m_attributeIDs);
			...
			else
			{
				...
				// begin read data.
				ResultCollection results = m_trend.ReadAttributes(
					item,
					m_attributeIDs,
					null,
					new ReadAttributesEventHandler(OnReadComplete),
					out m_request);
		...
		/// </summary>
		public void OnReadComplete(IRequest request, ItemAttributeCollection results)
		{
			...
			if (InvokeRequired)
			{
				BeginInvoke(new ReadAttributesEventHandler(OnReadComplete), new object[] { request, results });
				return;
			}
