...
namespace Opc.Hda.SampleClient
{
	...
	public class AnnotationValuesCtrl : System.Windows.Forms.UserControl
	{
		...
		/// </summary>
		public void Initialize(Opc.Hda.Server server, AnnotationValueCollection values)
		{
			m_server = server;
			m_item   = values;
			...
			if (values != null)
			{
				...
				foreach (AnnotationValue value in values)
				{
					AddListItem(value, -1);
				}
		...
		/// </summary>
		private object GetFieldValue(AnnotationValue value, int fieldID)
		{
			if (value != null)
			{
				switch (fieldID)
		...
		/// </summary>
		private void AddListItem(AnnotationValue value, int index)
		{
		...
		/// </summary>
		private void UpdateListItem(ListViewItem listItem, AnnotationValue value)
		{
			// set column values.
		...
		private void CopyMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				AnnotationValue value = (AnnotationValue)ValuesLV.SelectedItems[0].Tag;

				// create new item value.
				AnnotationValue copy = new AnnotationValueEditDlg().ShowDialog((AnnotationValue)value.Clone());
		...
		private void EditMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				// create new item value.
				AnnotationValue value = new AnnotationValueEditDlg().ShowDialog((AnnotationValue)ValuesLV.SelectedItems[0].Tag);
		...
		private void ImportValuesMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				AnnotationValueCollection values = new ReadAnnotationsDlg().ShowDialog(m_server, true);
