...
namespace Opc.Hda.SampleClient
{
	...
	public class AttributesViewCtrl : System.Windows.Forms.UserControl
	{
		...
		public void Initialize(Opc.Hda.Server server)
		{
			...
			if (server != null)
			{
				foreach (Attribute attribute in server.Attributes)
				{
		...
		/// </summary>
		public void Initialize(Opc.Hda.Server server, ItemAttributeCollection results)
		{
			m_server = server;
			...
			if (results != null)
			{
				foreach (AttributeValueCollection result in results)
				{
					AddListItem(result);
				}
		...
		/// </summary>
		public void Initialize(Opc.Hda.Server server, AttributeValueCollection values)
		{
			m_server = server;
		...
		/// Displays the values of an attribute.
		/// </summary>
		public void Initialize(Opc.Hda.Server server, int attributeIDs, ResultCollection results)
		{
			m_server = server;
		...
		public Attribute[] GetAttributes()
		{
			...
			{
				if (typeof(Attribute).IsInstanceOfType(listItem.Tag))
				{
					attributes.Add(listItem.Tag);
				}
			}

			return (Attribute[])attributes.ToArray(typeof(Attribute));
		}
		#endregion
		...
		{
			// display result code.
			if (typeof(IResult).IsInstanceOfType(attribute))
			{
				switch (fieldID)
				{								
					case RESULT: { return ((AttributeValueCollection)attribute).ResultID; }
				}
			}

			// displaying attribute descriptions. 
			if (typeof(Attribute).IsInstanceOfType(attribute))
			{
				if (AttributesLV.CheckBoxes)
				{
					switch (fieldID)
					{
						case NAME: { return ((Attribute)attribute).Name; }
					}
				}
				else
				{
					...
					{
						case NAME:        { return ((Attribute)attribute).Name;        }
						case DATA_TYPE:   { return ((Attribute)attribute).DataType;    }
						case DESCRIPTION: { return ((Attribute)attribute).Description; }
					}
				}
			}

			// displaying attribute results. 
			if (typeof(AttributeValueCollection).IsInstanceOfType(attribute))
			{
				AttributeValueCollection collection = (AttributeValueCollection)attribute;

				switch (fieldID)
				{					
					case NAME:        
					{ 
						Attribute description =	m_server.Attributes.Find(collection.AttributeID);
			...
			if (typeof(AttributeValue).IsInstanceOfType(attribute))
			{
				switch (fieldID)
				{					
					case TIMESTAMP: { return ((AttributeValue)attribute).Timestamp; }
					case VALUE:     { return ((AttributeValue)attribute).Value;     }
				}
			}
		...
		private void AddListItem(object attribute)
		{
			...
			{
				listItem.SubItems[ii].Text = Opc.Convert.ToString(GetFieldValue(attribute, ii));
			}
		...
		private void ViewMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (typeof(AttributeValueCollection).IsInstanceOfType(selection.Tag))
				{
					new AttributesViewDlg().ShowDialog(m_server, (AttributeValueCollection)selection.Tag);
				}

				// show description of attribute.
				else if(typeof(Attribute).IsInstanceOfType(selection.Tag))
				{
					Attribute attribute = (Attribute)selection.Tag;
