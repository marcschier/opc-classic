...
namespace Opc.Hda.SampleClient
{
	...
	public class TrendTreeCtrl : System.Windows.Forms.UserControl
	{
		...
		/// </summary>
		public delegate void TrendChangedEventHandler(Trend trend, ItemValueCollection[] values, bool replace);
		...
		/// </summary>
		public delegate void TrendSelectedEventHandler(Trend trend, Item item);
		...
		/// </summary>
		public void Initialize(Opc.Hda.Server server)
		{
			m_server = server;
			...
			foreach (Trend trend in server.Trends)
			{
		...
		/// </summary>
		private void AddTrend(TreeNode parent, Trend trend, bool expand)
		{
		...
		/// </summary>
		private void UpdateTrend(TreeNode node, Trend trend, ItemValueCollection[] values)
		{
		...
		/// </summary>
		private void UpdateItem(TreeNode node, Item item)
		{
			Aggregate aggregate = m_server.Aggregates.Find(item.AggregateID);
		...
		private bool IsServerNode(TreeNode node)
		{
			if (node != null)
			{
				if (typeof(Opc.Hda.Server).IsInstanceOfType(node.Tag))
				{
					return true;
				}
			}
		...
		private bool IsTrendNode(TreeNode node)
		{
			if (node != null)
			{
				if (typeof(Opc.Hda.Trend).IsInstanceOfType(node.Tag))
				{
					return true;
				}
			}
		...
		private bool IsItemNode(TreeNode node)
		{
			if (node != null)
			{
				if (typeof(Opc.Hda.Item).IsInstanceOfType(node.Tag))
				{
					return true;
				}
			}
		...
		/// </summary>
		private void HandleRequest(Opc.Hda.Server server, RequestType type)
		{
			switch (type)
			{
		...
		/// </summary>
		private void HandleRequest(Opc.Hda.Trend trend, RequestType type)
		{
			...
			switch (type)
			{
				...
				case RequestType.AdviseProcessed:
				{
					...
					trend.Subscribe(trend, new DataUpdateEventHandler(OnDataChanged));
				...
				case RequestType.PlaybackProcessed:
				{
					...
					// create a subscription.
					trend.Playback(trend, new DataUpdateEventHandler(OnDataChanged));
		...
		/// </summary>
		private void HandleRequest(Opc.Hda.Trend trend, Opc.Hda.Item item, RequestType type)
		{
			// used only if the request reads a new data set from the server.
			ItemValueCollection[] results = null;
			...
			switch (type)
			{
				case RequestType.ReadRaw:
				{
					results = trend.ReadRaw(new Item[] { item });
					break;
				}

				case RequestType.ReadProcessed:
				{
					results = trend.ReadProcessed(new Item[] { item });
					break;
				}

				case RequestType.ReadModified:
				{
					results = trend.ReadModified(new Item[] { item });
					break;
				}

				case RequestType.ReadAtTime:
				{
					results = trend.ReadAtTime(new Item[] { item });
					break;
				}
				...
				case RequestType.ReadAnnotations:
				{
					trend.ReadAnnotations(new Item[] { item });
					break;
				}
		...
		/// </summary>
		public void OnDataChanged(IRequest request, ItemValueCollection[] results)
		{	
			...
			if (InvokeRequired)
			{
				BeginInvoke(new DataUpdateEventHandler(OnDataChanged), new object[] { request, results });
				return;
			}
			...
			try
			{
				...
				if (results != null)
				{
					...
					Trend trend = (Trend)request.Handle;
					...
					if (trend != null)
					{
						if (trend.PlaybackActive)
						{
							...
							foreach (ItemValueCollection result in results)
							{
								if (result.Count > 0)
								{
									done = false;
		...
		private void TrendTV_MouseDown(object sender, System.Windows.Forms.MouseEventArgs e)
		{
			...
			if (typeof(Opc.Hda.Server).IsInstanceOfType(clickedNode.Tag))
			{
			...
			if (typeof(Trend).IsInstanceOfType(clickedNode.Tag))
			{
				Trend trend = (Trend)clickedNode.Tag;
			...
			if (typeof(Item).IsInstanceOfType(clickedNode.Tag))
			{
		...
		private void TrendTV_AfterSelect(object sender, System.Windows.Forms.TreeViewEventArgs e)
		{
			try
			{
				...
				if (TrendTV.SelectedNode != null)
				{
					...
					if (typeof(Trend).IsInstanceOfType(selection))
					{
						TrendSelected((Trend)selection, null);
						return;
					}

					if (typeof(Item).IsInstanceOfType(selection))
					{
						TrendSelected((Trend)TrendTV.SelectedNode.Parent.Tag, (Item)selection);
						return;
					}
				}
		...
		private void AddTrendMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (node == null || !typeof(Opc.Hda.Server).IsInstanceOfType(node.Tag))
				{
					return;
				}	

				// create trend.
				Trend trend = new TrendCreateDlg().ShowDialog(m_server);
		...
		private void EditTrendMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (node == null || !typeof(Opc.Hda.Trend).IsInstanceOfType(node.Tag))
				{
					return;
				}

				// edit trend.
				Trend trend = (Trend)node.Tag;
		...
		private void RemoveTrendMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				ItemIdentifier[] itemIDs = new ItemIdentifier[trend.Items.Count];

				for (int ii = 0; ii < itemIDs.Length; ii++)
				{
					itemIDs[ii] = new ItemIdentifier(trend.Items[ii]);
				}
		...
		private void AddItemsMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				ItemIdentifier[] items = new TrendCreateDlg().ShowDialog(trend);
		...
		private void EditItemMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (node == null || !typeof(Opc.Hda.Item).IsInstanceOfType(node.Tag))
				{
					return;
				}
				...
				Trend trend = (Trend)node.Parent.Tag;
		...
		{		
			try
			{
				...
				if (node.Parent == null || !typeof(Opc.Hda.Trend).IsInstanceOfType(node.Parent.Tag))
				{
					return;
				}
				...
				trend.RemoveItem((Item)node.Tag);
		...
		{		
			try
			{
				...
				{
					HandleRequest((Server)node.Tag, RequestType.ReadRaw);
					return;
				}

				if (IsTrendNode(TrendTV.SelectedNode))
				{
					HandleRequest((Trend)node.Tag, RequestType.ReadRaw);
					return;
				}

				if (IsItemNode(TrendTV.SelectedNode))
				{
					HandleRequest((Trend)node.Parent.Tag, (Item)node.Tag, RequestType.ReadRaw);
					return;
				}
			}
			catch (Exception exception)
		...
		{		
			try
			{
				...
				if (IsServerNode(node))
				{
					HandleRequest((Server)node.Tag, RequestType.ReadProcessed);
					return;
				}

				if (IsTrendNode(TrendTV.SelectedNode))
				{
					HandleRequest((Trend)node.Tag, RequestType.ReadProcessed);
					return;
				}

				if (IsItemNode(TrendTV.SelectedNode))
				{
					HandleRequest((Trend)node.Parent.Tag, (Item)node.Tag, RequestType.ReadProcessed);
					return;
				}
			}
			catch (Exception exception)
		...
		private void ReadModifiedMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (IsServerNode(node))
				{
					HandleRequest((Server)node.Tag, RequestType.ReadModified);
					return;
				}

				if (IsTrendNode(TrendTV.SelectedNode))
				{
					HandleRequest((Trend)node.Tag, RequestType.ReadModified);
					return;
				}

				if (IsItemNode(TrendTV.SelectedNode))
				{
					HandleRequest((Trend)node.Parent.Tag, (Item)node.Tag, RequestType.ReadModified);
					return;
				}
			}
			catch (Exception exception)
		...
		private void ReadAtTimeMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (IsServerNode(node))
				{
					HandleRequest((Server)node.Tag, RequestType.ReadAtTime);
					return;
				}

				if (IsTrendNode(TrendTV.SelectedNode))
				{
					HandleRequest((Trend)node.Tag, RequestType.ReadAtTime);
					return;
				}

				if (IsItemNode(TrendTV.SelectedNode))
				{
					HandleRequest((Trend)node.Parent.Tag, (Item)node.Tag, RequestType.ReadAtTime);
					return;
				}
			}
			catch (Exception exception)
		...
		private void ReadAttributesMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (IsServerNode(node))
				{
					HandleRequest((Server)node.Tag, RequestType.ReadAttributes);
					return;
				}

				if (IsItemNode(TrendTV.SelectedNode))
				{
					HandleRequest((Trend)node.Parent.Tag, (Item)node.Tag, RequestType.ReadAttributes);
					return;
				}
			}
			catch (Exception exception)
		...
		private void ReadAnnotationsMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (IsServerNode(node))
				{
					HandleRequest((Server)node.Tag, RequestType.ReadAnnotations);
					return;
				}

				if (IsTrendNode(TrendTV.SelectedNode))
				{
					HandleRequest((Trend)node.Tag, RequestType.ReadAnnotations);
					return;
				}

				if (IsItemNode(TrendTV.SelectedNode))
				{
					HandleRequest((Trend)node.Parent.Tag, (Item)node.Tag, RequestType.ReadAnnotations);
					return;
				}
			}
			catch (Exception exception)
		...
		private void InsertMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (IsServerNode(node))
				{
					HandleRequest((Server)node.Tag, RequestType.Insert);
					return;
				}

				if (IsTrendNode(TrendTV.SelectedNode))
				{
					HandleRequest((Trend)node.Tag, RequestType.Insert);
					return;
				}

				if (IsItemNode(TrendTV.SelectedNode))
				{
					HandleRequest((Trend)node.Parent.Tag, (Item)node.Tag, RequestType.Insert);
					return;
				}
			}
			catch (Exception exception)
		...
		private void InsertReplaceMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (IsServerNode(node))
				{
					HandleRequest((Server)node.Tag, RequestType.InsertReplace);
					return;
				}

				if (IsTrendNode(TrendTV.SelectedNode))
				{
					HandleRequest((Trend)node.Tag, RequestType.InsertReplace);
					return;
				}

				if (IsItemNode(TrendTV.SelectedNode))
				{
					HandleRequest((Trend)node.Parent.Tag, (Item)node.Tag, RequestType.InsertReplace);
					return;
				}
			}
			catch (Exception exception)
		...
		private void ReplaceMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (IsServerNode(node))
				{
					HandleRequest((Server)node.Tag, RequestType.Replace);
					return;
				}

				if (IsTrendNode(TrendTV.SelectedNode))
				{
					HandleRequest((Trend)node.Tag, RequestType.Replace);
					return;
				}

				if (IsItemNode(TrendTV.SelectedNode))
				{
					HandleRequest((Trend)node.Parent.Tag, (Item)node.Tag, RequestType.Replace);
					return;
				}
			}
			catch (Exception exception)
		...
		private void DeleteAtTimeMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (IsServerNode(node))
				{
					HandleRequest((Server)node.Tag, RequestType.DeleteAtTime);
					return;
				}

				if (IsTrendNode(TrendTV.SelectedNode))
				{
					HandleRequest((Trend)node.Tag, RequestType.DeleteAtTime);
					return;
				}

				if (IsItemNode(TrendTV.SelectedNode))
				{
					HandleRequest((Trend)node.Parent.Tag, (Item)node.Tag, RequestType.DeleteAtTime);
					return;
				}
			}
			catch (Exception exception)
		...
		private void DeleteRawMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (IsServerNode(node))
				{
					HandleRequest((Server)node.Tag, RequestType.DeleteRaw);
					return;
				}

				if (IsTrendNode(TrendTV.SelectedNode))
				{
					HandleRequest((Trend)node.Tag, RequestType.DeleteRaw);
					return;
				}

				if (IsItemNode(TrendTV.SelectedNode))
				{
					HandleRequest((Trend)node.Parent.Tag, (Item)node.Tag, RequestType.DeleteRaw);
					return;
				}
			}
			catch (Exception exception)
		...
		{		
			try
			{
				...
				if (IsServerNode(node))
				{
					HandleRequest((Server)node.Tag, RequestType.InsertAnnotations);
					return;
				}

				if (IsTrendNode(TrendTV.SelectedNode))
				{
					HandleRequest((Trend)node.Tag, RequestType.InsertAnnotations);
					return;
				}

				if (IsItemNode(TrendTV.SelectedNode))
				{
					HandleRequest((Trend)node.Parent.Tag, (Item)node.Tag, RequestType.InsertAnnotations);
					return;
				}
			}
			catch (Exception exception)
		...
		private void SubscribeRawMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (IsTrendNode(TrendTV.SelectedNode))
				{
					HandleRequest((Trend)node.Tag, RequestType.AdviseRaw);
					return;
				}
			}
			catch (Exception exception)
		...
		private void SubscribeProcessedMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (IsTrendNode(TrendTV.SelectedNode))
				{
					HandleRequest((Trend)node.Tag, RequestType.AdviseProcessed);
					return;
				}
			}
			catch (Exception exception)
		...
		private void SubscribeCancelMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (IsTrendNode(TrendTV.SelectedNode))
				{
					((Trend)node.Tag).SubscribeCancel();
					return;
				}
			}
			catch (Exception exception)
		...
		private void PlaybackRawMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (IsTrendNode(TrendTV.SelectedNode))
				{
					HandleRequest((Trend)node.Tag, RequestType.PlaybackRaw);
					return;
				}
			}
			catch (Exception exception)
		...
		private void PlaybackProcessedMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (IsTrendNode(TrendTV.SelectedNode))
				{
					HandleRequest((Trend)node.Tag, RequestType.PlaybackProcessed);
					return;
				}
			}
			catch (Exception exception)
