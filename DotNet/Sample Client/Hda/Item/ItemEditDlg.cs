...
{	
	...
	public class ItemEditDlg : System.Windows.Forms.Form
	{
		...
		/// </summary>
		public bool ShowDialog(Opc.Hda.Server server, Item item)
		{
			...
			foreach (Aggregate aggregate in server.Aggregates)
			{
				AggregateCB.Items.Add(aggregate);

				if (aggregate.ID == item.AggregateID)
				{
					AggregateCB.SelectedItem = aggregate;
				}
			}

			AggregateSpecifiedCB.Checked = (item.AggregateID != AggregateID.NOAGGREGATE);
			...
			// update the item.
			item.AggregateID = AggregateID.NOAGGREGATE;

			if (AggregateSpecifiedCB.Checked && AggregateCB.SelectedItem != null)
			{
				item.AggregateID = ((Aggregate)AggregateCB.SelectedItem).ID;
			}
		...
		/// </summary>
		public bool ShowDialog(ItemIdentifier item)
		{
			if (item == null)   throw new ArgumentNullException("item");
