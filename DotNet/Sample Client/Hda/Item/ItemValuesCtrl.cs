...
namespace Opc.Hda.SampleClient
{
	...
	public class ItemValuesCtrl : System.Windows.Forms.UserControl
	{
		...
		/// </summary>
		public void Initialize(Opc.Hda.Server server, ItemValueCollection values)
		{
			m_server = server;
			m_item   = values;
			...
			if (values != null)
			{
				...
				foreach (ItemValue value in values)
				{
					AddListItem(value, -1);
				}
		...
		/// The current item id.
		/// </summary>
		private Opc.ItemIdentifier m_item = null;
		...
		/// </summary>
		private double CalculateUnits(ItemValueCollection values)
		{
			...
			double              units)
		{
			...
			{
				ItemValue value = (ItemValue)points[ii];
		...
		private void DrawGraph()
		{
			...
			ItemValueCollection values = GetValues();
		...
		/// </summary>
		private object GetFieldValue(ItemValue value, int fieldID)
		{
			if (value != null)
			{
				switch (fieldID)
				...
				if (typeof(ModifiedValue).IsInstanceOfType(value))
				{
					switch (fieldID)
					{
						case MODIFICATION_TIME: { return ((ModifiedValue)value).ModificationTime; }
						case EDIT_TYPE:         { return ((ModifiedValue)value).EditType;         }
						case USER:              { return ((ModifiedValue)value).User;             }
					}
				}
			}
		...
		/// </summary>
		private void AddListItem(ItemValue value, int index)
		{
		...
		/// </summary>
		private void UpdateListItem(ListViewItem listItem, ItemValue value)
		{
			// set column values.
			for (int ii = 0; ii < listItem.SubItems.Count; ii++)
			{
				listItem.SubItems[ii].Text = Opc.Convert.ToString(GetFieldValue(value, ii));
			}
		...
		private void AddMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				value.HistorianQuality = Quality.Raw;
		...
		private void CopyMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				ItemValue value = (ItemValue)ValuesLV.SelectedItems[0].Tag;

				// create new item value.
				ItemValue copy = new ItemValueEditDlg().ShowDialog((ItemValue)value.Clone());
		...
		private void EditMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				// create new item value.
				ItemValue value = new ItemValueEditDlg().ShowDialog((ItemValue)ValuesLV.SelectedItems[0].Tag);
		...
		private void ImportValuesMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				ItemValueCollection values = new ReadValuesDlg().ShowDialog(m_server, RequestType.ReadRaw, true);
