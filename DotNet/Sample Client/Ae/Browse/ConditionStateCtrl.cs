//============================================================================
// TITLE: ConditionStateCtrl.cs
//
// CONTENTS:
// 
// A control used to display the state of a condition.
//
...
namespace Opc.Ae.SampleClient
{
	...
	public class ConditionStateCtrl : System.Windows.Forms.UserControl
	{
		...
		/// </summary>
		public void ShowCondition(Attribute[] attributes, Condition condition)
		{
			...
			if ((condition.State & (int)ConditionState.Active) != 0)
			{
				StateTB.Text += ConditionState.Active.ToString();
			}

			if ((condition.State & (int)ConditionState.Enabled) != 0)
			{
				if (StateTB.Text != "") StateTB.Text += " AND ";
				StateTB.Text += ConditionState.Enabled.ToString();
			}

			if ((condition.State & (int)ConditionState.Acknowledged) != 0)
			{
				if (StateTB.Text != "") StateTB.Text += " AND ";
				StateTB.Text += ConditionState.Acknowledged.ToString();
			}

			QualityTB.Text                = Opc.Convert.ToString(condition.Quality);
			...
			ConditionLastActiveTB.Text    = Opc.Convert.ToString(condition.CondLastActive);
			ConditionLastInactiveTB.Text  = Opc.Convert.ToString(condition.CondLastInactive);
			SubConditionLastActiveTB.Text = Opc.Convert.ToString(condition.SubCondLastActive);
			LastAcknowledgedTB.Text       = Opc.Convert.ToString(condition.LastAckTime);
			...
			for (int ii = 0; ii < condition.Attributes.Count; ii++)
			{
				...
				item.SubItems.Add(Opc.Convert.ToString(condition.Attributes[ii].Value));
				item.SubItems.Add(Opc.Convert.ToString(condition.Attributes[ii].ResultID));
