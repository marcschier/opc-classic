...
namespace Opc.Ae.SampleClient
{
	...
	public class GetEnabledStateDlg : System.Windows.Forms.Form
	{
		...
		/// </summary>
		public void ShowDialog(Opc.Ae.Server server, BrowseElement[] elements)
		{
			...
			{
				if (elements[ii].NodeType == BrowseType.Area)
				{
		...
		private void GetEnabledState()
		{
			...
			try
			{
				EnabledStateResult[] results = m_server.GetEnableStateByArea((string[])m_areas.ToArray(typeof(string)));

				for (int ii = 0; ii < results.Length; ii++)
				{
					ListViewItem item = new ListViewItem((string)m_areas[ii], Resources.IMAGE_CLOSED_BLUE_FOLDER);

					item.SubItems.Add(Opc.Convert.ToString(results[ii].Enabled));
					item.SubItems.Add(Opc.Convert.ToString(results[ii].EffectivelyEnabled));
					item.SubItems.Add(Opc.Convert.ToString(results[ii].ResultID));
			...
			try
			{
				EnabledStateResult[] results = m_server.GetEnableStateBySource((string[])m_sources.ToArray(typeof(string)));
