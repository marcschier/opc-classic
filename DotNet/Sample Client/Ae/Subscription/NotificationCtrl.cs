//============================================================================
// TITLE: NotificationCtrl.cs
//
// CONTENTS:
// 
// A control used to display the state of a notification.
//
...
namespace Opc.Ae.SampleClient
{
	...
	public class NotificationCtrl : System.Windows.Forms.UserControl
	{
		...
		/// </summary>
		public void ShowNotification(Opc.Ae.Subscription subscription, EventNotification notification)
		{
			...
			try
			{
				Category[] categories = subscription.Server.QueryEventCategories((int)notification.EventType);
			...
			try
			{
				...
				Attribute[] descriptions = subscription.Server.QueryEventAttributes(notification.EventCategory);
			...
			TimeTB.Text             = Opc.Convert.ToString(notification.Time);
			MessageTB.Text          = notification.Message;
			EventTypeTB.Text        = Opc.Convert.ToString(notification.EventType);
			...
			NewStateTB.Text         = "";
			AckRequiredTB.Text      = Opc.Convert.ToString(notification.AckRequired);
			QualityTB.Text          = Opc.Convert.ToString(notification.Quality);
			ActiveTimeTB.Text       = Opc.Convert.ToString(notification.ActiveTime);
			ActorTB.Text            = notification.ActorID;

			// convert state to a string.
			if ((notification.NewState & (int)ConditionState.Active) != 0)
			{
				NewStateTB.Text += ConditionState.Active.ToString();
			}

			if ((notification.NewState & (int)ConditionState.Enabled) != 0)
			{
				if (NewStateTB.Text != "") NewStateTB.Text += " AND ";
				NewStateTB.Text += ConditionState.Enabled.ToString();
			}

			if ((notification.NewState & (int)ConditionState.Acknowledged) != 0)
			{
				if (NewStateTB.Text != "") NewStateTB.Text += " AND ";
				NewStateTB.Text += ConditionState.Acknowledged.ToString();
			}
			...
			{
				Attribute attribute = (ii < attributes.Count)?(Attribute)attributes[ii]:null;

				ListViewItem item = new ListViewItem((attribute != null)?attribute.Name:"Unknown");

				item.SubItems.Add(Opc.Convert.ToString(notification.Attributes[ii]));
