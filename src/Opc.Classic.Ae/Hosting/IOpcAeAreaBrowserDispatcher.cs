...
public interface IOpcAeAreaBrowserDispatcher
{
    ...
    /// <summary>Browses the current cursor and returns area or source names matching the filter.</summary>
    Task<string[]> BrowseAreasAsync(int browseFilterType, string filterCriteria, CancellationToken cancellationToken = default);

    /// <summary>Returns a fully-qualified area name for a name relative to the current cursor.</summary>
    Task<string> GetQualifiedAreaNameAsync(string areaName, CancellationToken cancellationToken = default);

    /// <summary>Returns a fully-qualified source name for a name relative to the current cursor.</summary>
