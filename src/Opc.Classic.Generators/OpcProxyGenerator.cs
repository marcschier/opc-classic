...
";
        ...
        string responseSpanLocal)
    {
        ...
        foreach (var parameter in method.Parameters)
        {
            ...
            // [out, size_is(,N)] FILETIME** with [OpcFileTimeElements] tag:
            // each element is 2x DWORD (4-byte aligned), not 8-byte Int64.
            if (parameter.FileTimeElements && IsLongArrayMarshallingType(parameter.MarshallingType))
            {
                ...
                sb.Append(statementIndent).Append("var ").Append(ResponseValueLocalName(itemIndex))
    ...
    {
        public ParameterModel(string name, string resultMemberName, string declaredType, string marshallingType, bool isCancellationToken, RefKind refKind, bool isOpcInterface, bool isUniquePointer = false, string? underlyingValueType = null, bool emitArrayCount = false, bool deferredElements = false, bool fileTimeElements = false) { Name = name; ResultMemberName = resultMemberName; DeclaredType = declaredType; MarshallingType = marshallingType; IsCancellationToken = isCancellationToken; RefKind = refKind; IsOpcInterface = isOpcInterface; IsUniquePointer = isUniquePointer; UnderlyingValueType = underlyingValueType; EmitArrayCount = emitArrayCount; DeferredElements = deferredElements; FileTimeElements = fileTimeElements; }
