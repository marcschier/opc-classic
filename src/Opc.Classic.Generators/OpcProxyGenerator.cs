...
#pragma warning disable MA0051 // Shape extraction stays together so client/server model parity is auditable.
    private static MethodModel CreateMethodModel(IMethodSymbol method)
    {
        ...
        return new MethodModel(method.Name, method.ReturnType.ToDisplayString(TypeDisplayFormat), TypeParameterList(method.TypeParameters), ParameterList(method.Parameters), ConstraintClauses(method.TypeParameters), outParameters.ToImmutable(), parameterNames.ToImmutable(), parameterModels, HasOpcMethodAttribute(method), hasRefOrOutParameter, HasGenerateMultiOutRecordAttribute(method) || outParameterCount >= 2, taskReturnKind, taskResultType, taskResultMarshallingType, taskResultIsOpcInterface, taskResultIsOpcInterfaceArray, taskResultOpcInterfaceArrayElementType, taskResultIsUniquePointer, taskResultIidIsParameterName, taskResultArrayCountParameterName, unsupportedMarshallingType, unsupportedRefOutMarshallingType, unsupportedMarshallingKind, unsupportedParameterName, unsupportedSignatureReason, invalidCorrelationMember, invalidCorrelationReason, cancellationTokenParameterName, declaringNamespace, method.Locations.IsDefaultOrEmpty ? Location.None : method.Locations[0]);
    }
