...
#pragma warning restore MA0051
    ...
    private static void ValidateCorrelations(IMethodSymbol method, ImmutableArray<ParameterModel> parameters, TaskReturnKind taskReturnKind, bool taskResultIsOpcInterface, bool taskResultIsOpcInterfaceArray, string? taskResultIidIsParameterName, string? taskResultArrayCountParameterName, out string? invalidMember, out string? invalidReason)
    {
        invalidMember = null;
        invalidReason = null;
        ...
        if (taskResultArrayCountParameterName is not null && !ValidateArrayCorrelation(method, taskResultArrayCountParameterName, IsTaskResultArray(method), false, taskReturnKind, out invalidReason))
        {
            invalidMember = "ReturnValue";
        }
    }
    ...
    private static bool ValidateArrayCorrelation(IMethodSymbol method, string sourceName, bool isArray, bool targetIsRequestValue, TaskReturnKind taskReturnKind, out string? reason)
    {
        ...
        if (!IsCountType(source.Type)) { reason = $"array-count source '{sourceName}' must be System.Int32 or System.UInt32"; return false; }
...
#pragma warning restore MA0051
    ...
    private static void EmitResponseCorrelationChecks(StringBuilder sb, string statementIndent, MethodModel method)
    {
        ...
        {
            EmitArrayCountCheck(sb, statementIndent, ResponseValueLocalName(0), CorrelationSourceExpression(method, taskSource), method.Name, "ReturnValue");
        }
        foreach (var parameter in method.Parameters)
        {
            if (!parameter.IsResponseValue)
    ...
    private static string CorrelationSourceExpression(MethodModel method, string sourceName)
    {
        ...
        foreach (var parameter in method.Parameters)
        {
            ...
            {
                return parameter.IsResponseValue ? ResponseValueLocalName(itemIndex) : parameter.Name;
            }
            if (parameter.IsResponseValue)
            {
                itemIndex++;
