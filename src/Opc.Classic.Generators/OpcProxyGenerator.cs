...
public sealed class OpcProxyGenerator : IIncrementalGenerator
{
    ...
    private const string OpcIidIsAttributeFullName = AttributeNamespace + ".OpcIidIsAttribute";
    private const string OpcArrayCountAttributeFullName = AttributeNamespace + ".OpcArrayCountAttribute";
    private const string OpcEnumeratorArrayAttributeFullName = AttributeNamespace + ".OpcEnumeratorArrayAttribute";
    private const string OpcProxyIgnoreAttributeFullName = AttributeNamespace + ".OpcProxyIgnoreAttribute";
...
namespace Opc.Classic.Generators
{
    ...
    internal sealed class OpcProxyIgnoreAttribute : global::System.Attribute { }
    ...
    /// <summary>
    /// Marks a <c>long[]</c> array parameter as carrying Windows FILETIME
    /// values (each element is two consecutive 32-bit halves with 4-byte
    /// alignment) rather than 8-byte Int64. Used for IDL
    /// <c>[size_is(N)] FILETIME *p</c> arrays in IOPCItemIO::Read,
    /// IOPCSyncIO::Read, etc.
    /// </summary>
    [global::System.AttributeUsage(global::System.AttributeTargets.Parameter, AllowMultiple = false, Inherited = false)]
    internal sealed class OpcFileTimeElementsAttribute : global::System.Attribute { }

    /// <summary>
    /// Marks an <c>OpcVariant[]</c> array parameter as carrying VARIANT
    /// elements in the MS-OAUT §2.2.29.2 NDR transmission form (wireVARIANT
    /// header + body + pad-to-8 trailer per element) rather than back-to-back
    /// inline top-level wireVARIANTs. This matches the wire format real DCOM
    /// peers (e.g. spec-compliant OPC DA servers) emit for IDL signatures
    /// like <c>[out, size_is(,N)] VARIANT** ppvValues</c>.
    /// </summary>
    [global::System.AttributeUsage(global::System.AttributeTargets.Parameter, AllowMultiple = false, Inherited = false)]
    internal sealed class OpcVariantElementsAttribute : global::System.Attribute { }

    /// <summary>
    /// Marks a <c>string</c> parameter as a top-level reference pointer
    /// (per DCE 1.1 §4.2.2.7: top-level pointers in operations default to
    /// <c>[ref]</c> regardless of <c>pointer_default</c>). The wire layout
    /// becomes a bare conformant-varying string (max_count + offset +
    /// actual_count + chars) with no leading referent ID. Required for OPC
    /// IDL signatures like <c>[in, string] LPWSTR szItemID</c> in
    /// <c>IOPCBrowse::Browse</c> where MIDL flags reveal the parameter is
    /// <c>simple ref</c>.
    /// </summary>
    [global::System.AttributeUsage(global::System.AttributeTargets.Parameter, AllowMultiple = false, Inherited = false)]
    internal sealed class OpcRefStringAttribute : global::System.Attribute { }
}
";
    ...
    private static readonly Dictionary<string, CodecEmitter> Codecs =
        new(System.StringComparer.Ordinal)
        {
            ...
            { "global::Opc.Classic.Dx.DxItemIdentifier", StaticCodec("global::Opc.Classic.Dx.Ndr.NdrOpcDxItemIdentifierCodec") },
            { "global::Opc.Classic.Dx.DxSourceServer", StaticCodec("global::Opc.Classic.Dx.Ndr.NdrOpcDxSourceServerCodec") },
            { "global::Opc.Classic.Dx.DxGeneralResponse", StaticCodec("global::Opc.Classic.Dx.Ndr.NdrOpcDxGeneralResponseCodec") },
            { "global::Opc.Classic.Dx.DxConnection", StaticCodec("global::Opc.Classic.Dx.Ndr.NdrOpcDxConnectionCodec") },
            { "global::Opc.Classic.Dx.DxConnectionQueryResult", StaticCodec("global::Opc.Classic.Dx.Ndr.NdrOpcDxConnectionQueryResultCodec") },
            { "global::Opc.Classic.Dx.DxUpdateConnectionsResult", StaticCodec("global::Opc.Classic.Dx.Ndr.NdrOpcDxUpdateConnectionsResultCodec") },
            { "global::Opc.Classic.Dx.DxDeleteConnectionsResult", StaticCodec("global::Opc.Classic.Dx.Ndr.NdrOpcDxDeleteConnectionsResultCodec") },
    ...
    private static readonly DiagnosticDescriptor InvalidCorrelationDescriptor = new(
        id: "OPCGEN011",
        title: "OpcMethod has an invalid wire correlation",
        messageFormat: "Method '{0}' has invalid correlation on '{1}': {2}",
        category: DiagnosticCategory,
        defaultSeverity: DiagnosticSeverity.Warning,
        isEnabledByDefault: true,
        helpLinkUri: DiagnosticsHelpLinkUri);
    ...
    private static ImmutableArray<MethodModel> CollectMethods(INamedTypeSymbol interfaceSymbol)
    {
        ...
        foreach (var member in interfaceSymbol.GetMembers())
        {
            if (member is IMethodSymbol { MethodKind: MethodKind.Ordinary } method &&
                !HasAttribute(method, OpcProxyIgnoreAttributeFullName))
            {
...
#pragma warning disable MA0051 // Shape extraction stays together so client/server model parity is auditable.
    private static MethodModel CreateMethodModel(IMethodSymbol method)
    {
        ...
        foreach (var parameter in method.Parameters)
        {
            ...
            string? iidIsParameterName = GetCorrelationParameterName(parameter, OpcIidIsAttributeFullName);
            string? arrayCountParameterName = GetCorrelationParameterName(parameter, OpcArrayCountAttributeFullName);
            bool isOpcInterfaceArray = TryGetInterfaceArrayElement(parameter.Type, out string? opcInterfaceArrayElementType);
            ...
            parameters.Add(new ParameterModel(
                ...
                refString,
                iidIsParameterName,
                arrayCountParameterName,
                isOpcInterfaceArray,
                opcInterfaceArrayElementType));
        }
        ...
        string? taskResultOpcInterfaceArrayElementType = null;
        bool taskResultIsOpcInterfaceArray = TryGetTaskResultSymbol(method.ReturnType, out taskResultSymbol) &&
            TryGetInterfaceArrayElement(taskResultSymbol, out taskResultOpcInterfaceArrayElementType);
        string? taskResultIidIsParameterName = GetReturnCorrelationParameterName(method, OpcIidIsAttributeFullName);
        string? taskResultArrayCountParameterName = GetReturnCorrelationParameterName(method, OpcArrayCountAttributeFullName);
        string? taskResultArrayCountMemberName = GetReturnCorrelationMemberName(method, OpcArrayCountAttributeFullName);
        bool taskResultEmitArrayCount = HasReturnAttribute(method, OpcEmitArrayCountAttributeFullName);
        GetEnumeratorArray(method, out string? taskResultEnumeratorCountParameterName, out bool taskResultEnumeratorIsVarying);
        bool taskResultIsUniquePointer = HasReturnUniquePointerAttribute(method) ||
            taskResultIsOpcInterface ||
            taskResultIsOpcInterfaceArray ||
            (taskResultIidIsParameterName is not null && IsOpcInterfaceRefType(taskResultMarshallingType ?? string.Empty)) ||
            (method.Name.StartsWith("Clone", System.StringComparison.Ordinal) && IsOpcInterfaceRefType(taskResultMarshallingType ?? string.Empty));
        ...
        ImmutableArray<ParameterModel> parameterModels = parameters.ToImmutable();
        string? unsupportedMarshallingType = FindUnsupportedMarshallingType(parameterModels, taskReturnKind, taskResultMarshallingType, taskResultIsOpcInterface, taskResultIsOpcInterfaceArray, declaringNamespace, out string? unsupportedRefOutMarshallingType, out UnsupportedMarshallingKind unsupportedMarshallingKind, out string? unsupportedParameterName);
        ValidateCorrelations(
            ...
            taskResultIsOpcInterface,
            taskResultIsOpcInterfaceArray,
            taskResultIidIsParameterName,
            taskResultArrayCountParameterName,
            taskResultArrayCountMemberName,
            taskResultEnumeratorCountParameterName,
            out string? invalidCorrelationMember,
            out string? invalidCorrelationReason);
        bool generateMultiOutRecord = HasGenerateMultiOutRecordAttribute(method) || outParameterCount >= 2;
        return new MethodModel(method.Name, method.ReturnType.ToDisplayString(TypeDisplayFormat), TypeParameterList(method.TypeParameters), ParameterList(method.Parameters), ConstraintClauses(method.TypeParameters), outParameters.ToImmutable(), parameterNames.ToImmutable(), parameterModels, HasOpcMethodAttribute(method), hasRefOrOutParameter, generateMultiOutRecord, GetMultiOutRecordName(method), taskReturnKind, taskResultType, taskResultMarshallingType, taskResultIsOpcInterface, taskResultIsOpcInterfaceArray, taskResultOpcInterfaceArrayElementType, taskResultIsUniquePointer, taskResultIidIsParameterName, taskResultArrayCountParameterName, taskResultArrayCountMemberName, taskResultEmitArrayCount, taskResultEnumeratorCountParameterName, taskResultEnumeratorIsVarying, unsupportedMarshallingType, unsupportedRefOutMarshallingType, unsupportedMarshallingKind, unsupportedParameterName, unsupportedSignatureReason, invalidCorrelationMember, invalidCorrelationReason, cancellationTokenParameterName, declaringNamespace, method.Locations.IsDefaultOrEmpty ? Location.None : method.Locations[0]);
    }
#pragma warning restore MA0051
    ...
    private static string? GetCorrelationParameterName(IParameterSymbol parameter, string attributeFullName) => GetCorrelationParameterName(parameter.GetAttributes(), attributeFullName);
    private static string? GetReturnCorrelationParameterName(IMethodSymbol method, string attributeFullName) => GetCorrelationParameterName(method.GetReturnTypeAttributes(), attributeFullName);
    private static string? GetReturnCorrelationMemberName(IMethodSymbol method, string attributeFullName) => GetCorrelationMemberName(method.GetReturnTypeAttributes(), attributeFullName);

    private static string? GetCorrelationParameterName(ImmutableArray<AttributeData> attributes, string attributeFullName)
    {
        foreach (var attr in attributes)
        {
            if (attr.AttributeClass is not null &&
                string.Equals(attr.AttributeClass.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat), "global::" + attributeFullName, System.StringComparison.Ordinal))
            {
                return attr.ConstructorArguments.Length >= 1 && attr.ConstructorArguments[0].Value is string parameterName ? parameterName : string.Empty;
            }
        }
        return null;
    }

    private static string? GetCorrelationMemberName(ImmutableArray<AttributeData> attributes, string attributeFullName)
    {
        foreach (var attr in attributes)
        {
            if (attr.AttributeClass is not null &&
                string.Equals(attr.AttributeClass.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat), "global::" + attributeFullName, System.StringComparison.Ordinal))
            {
                return attr.ConstructorArguments.Length >= 2 && attr.ConstructorArguments[1].Value is string memberName
                    ? memberName
                    : null;
            }
        }
    ...
    private static bool HasReturnAttribute(IMethodSymbol method, string attributeFullName) =>
        HasAttribute(method.GetReturnTypeAttributes(), attributeFullName);

    private static bool HasAttribute(ImmutableArray<AttributeData> attributes, string attributeFullName)
    {
        foreach (var attr in attributes)
        {
            if (attr.AttributeClass is not null &&
    ...
    private static void GetEnumeratorArray(
        IMethodSymbol method,
        out string? requestedCountParameterName,
        out bool conformantVarying)
    {
        requestedCountParameterName = null;
        conformantVarying = false;
        foreach (var attr in method.GetReturnTypeAttributes())
        {
            if (attr.AttributeClass is null ||
                !string.Equals(attr.AttributeClass.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat), "global::" + OpcEnumeratorArrayAttributeFullName, System.StringComparison.Ordinal))
            {
                continue;
            }

            requestedCountParameterName = attr.ConstructorArguments.Length >= 1
                ? attr.ConstructorArguments[0].Value as string ?? string.Empty
                : string.Empty;
            conformantVarying = attr.ConstructorArguments.Length >= 2 &&
                attr.ConstructorArguments[1].Value is true;
            return;
        }
    }

    private static bool TryGetInterfaceArrayElement(ITypeSymbol type, out string? elementType)
    {
        if (type is IArrayTypeSymbol { Rank: 1 } array &&
            (IsOpcInterfaceType(array.ElementType) || IsOpcInterfaceRefType(array.ElementType.ToDisplayString(CodecTypeDisplayFormat))))
        {
            elementType = array.ElementType.ToDisplayString(TypeDisplayFormat);
            return true;
        }
        elementType = null;
        return false;
    ...
    private static void ValidateCorrelations(
        ...
        bool taskResultIsOpcInterfaceArray,
        string? taskResultIidIsParameterName,
        string? taskResultArrayCountParameterName,
        string? taskResultArrayCountMemberName,
        string? taskResultEnumeratorCountParameterName,
        ...
        out string? invalidReason)
    {
        ...
        {
            if (parameter.IidIsParameterName is not null && !ValidateIidCorrelation(method, parameter.IidIsParameterName, parameter.IsOpcInterface || IsOpcInterfaceRefType(parameter.MarshallingType) || parameter.IsOpcInterfaceArray, out invalidReason))
            {
                invalidMember = parameter.Name;
                return;
            }
            if (parameter.ArrayCountParameterName is not null && !ValidateArrayCorrelation(method, parameter.ArrayCountParameterName, parameter.MarshallingType.EndsWith("[]", System.StringComparison.Ordinal), parameter.IsRequestValue, taskReturnKind, out invalidReason))
            {
                invalidMember = parameter.Name;
                return;
            }
            if (parameter.IsOpcInterfaceArray && parameter.IsRequestValue)
            {
                invalidMember = parameter.Name;
                invalidReason = "interface-pointer arrays are supported only as response values";
                return;
            }
        }
        if (taskResultIidIsParameterName is not null && !ValidateIidCorrelation(method, taskResultIidIsParameterName, taskResultIsOpcInterface || taskResultIsOpcInterfaceArray || IsTaskResultInterfaceRef(method), out invalidReason))
        {
            invalidMember = "ReturnValue";
            return;
        }
        bool taskResultCorrelationIsArray = IsTaskResultArray(method) ||
            (taskResultArrayCountMemberName is not null &&
             HasOneDimensionalArrayMember(TaskResultTypeSymbol(method), taskResultArrayCountMemberName));
        if (taskResultArrayCountParameterName is not null && !ValidateArrayCorrelation(method, taskResultArrayCountParameterName, taskResultCorrelationIsArray, false, taskReturnKind, out invalidReason))
        {
            invalidMember = "ReturnValue";
            return;
        }
        if (taskResultEnumeratorCountParameterName is not null &&
            !ValidateEnumeratorArray(method, taskResultEnumeratorCountParameterName, out invalidReason))
        {
            invalidMember = "ReturnValue";
        }
    }

    private static bool ValidateEnumeratorArray(IMethodSymbol method, string sourceName, out string? reason)
    {
        if (!IsTaskResultArray(method))
        {
            reason = "[OpcEnumeratorArray] is valid only on Task<T[]> return values";
            return false;
        }
        IParameterSymbol? source = FindParameter(method, sourceName);
        if (source is null)
        {
            reason = $"enumerator requested-count parameter '{sourceName}' was not found";
            return false;
        }
        if (!IsCountType(source.Type) || source.RefKind is RefKind.Out or RefKind.Ref)
        {
            reason = $"enumerator requested-count source '{sourceName}' must be an input System.Int32 or System.UInt32";
            return false;
        }
        reason = null;
        return true;
    ...
    private static bool ValidateIidCorrelation(IMethodSymbol method, string sourceName, bool isInterfacePointer, out string? reason)
    {
        if (!isInterfacePointer) { reason = "[OpcIidIs] is valid only on interface-pointer values"; return false; }
        IParameterSymbol? source = FindParameter(method, sourceName);
        if (source is null) { reason = $"iid_is source parameter '{sourceName}' was not found"; return false; }
        if (!string.Equals(source.Type.ToDisplayString(CodecTypeDisplayFormat), "global::System.Guid", System.StringComparison.Ordinal) || source.RefKind is RefKind.Out or RefKind.Ref)
        {
            reason = $"iid_is source '{sourceName}' must be an input System.Guid";
            return false;
        }
        reason = null;
        return true;
    ...
    private static bool ValidateArrayCorrelation(IMethodSymbol method, string sourceName, bool isArray, bool targetIsRequestValue, TaskReturnKind taskReturnKind, out string? reason)
    {
        if (!isArray) { reason = "[OpcArrayCount] is valid only on one-dimensional arrays"; return false; }
        if (string.Equals(sourceName, "ReturnValue", System.StringComparison.Ordinal))
        {
            if (targetIsRequestValue) { reason = "request arrays cannot correlate to ReturnValue"; return false; }
            if (taskReturnKind != TaskReturnKind.TaskOfT || !IsCountType(TaskResultTypeSymbol(method))) { reason = "ReturnValue count correlation requires Task<int> or Task<uint>"; return false; }
            reason = null;
            return true;
        }
        IParameterSymbol? source = FindParameter(method, sourceName);
        if (source is null) { reason = $"array-count source parameter '{sourceName}' was not found"; return false; }
        if (!IsCountType(source.Type) && source.Type is not IArrayTypeSymbol { Rank: 1 })
        {
            reason = $"array-count source '{sourceName}' must be System.Int32, System.UInt32, or a one-dimensional array";
            return false;
        }
        if (targetIsRequestValue && source.RefKind == RefKind.Out) { reason = $"request array-count source '{sourceName}' must be an input value"; return false; }
        reason = null;
        return true;
    }
    ...
    private static bool IsCountType(ITypeSymbol? type) => type?.SpecialType is SpecialType.System_Int32 or SpecialType.System_UInt32;
    private static bool HasOneDimensionalArrayMember(ITypeSymbol? type, string memberName) =>
        type?.GetMembers(memberName).OfType<IPropertySymbol>().Any(static property => property.Type is IArrayTypeSymbol { Rank: 1 }) == true;
    private static ITypeSymbol? TaskResultTypeSymbol(IMethodSymbol method) => TryGetTaskResultSymbol(method.ReturnType, out var resultType) ? resultType : null;
    private static bool IsTaskResultArray(IMethodSymbol method) => TaskResultTypeSymbol(method) is IArrayTypeSymbol { Rank: 1 };
    private static bool IsTaskResultInterfaceRef(IMethodSymbol method) => TaskResultTypeSymbol(method) is { } resultType && IsOpcInterfaceRefType(resultType.ToDisplayString(CodecTypeDisplayFormat));
    ...
    private static void Emit(SourceProductionContext spc, InterfaceModel? maybeModel)
    {
        ...
        sb.Append(indent).AppendLine("    }");
        EmitProxyHelperMethods(sb, indent, model);
    ...
    private static void EmitProxyHelperMethods(StringBuilder sb, string indent, InterfaceModel model)
    {
        if (!ModelHasInterfacePointerArrays(model))
        {
            return;
        }
        sb.AppendLine();
        sb.Append(indent).AppendLine("    private static global::Opc.Classic.Dcom.IOpcInterfaceRef? ReadMInterfacePointerBody(ref global::Opc.Classic.Ndr.NdrReader reader)");
        sb.Append(indent).AppendLine("    {");
        sb.Append(indent).AppendLine("        _ = reader.ReadUInt32();");
        sb.Append(indent).AppendLine("        uint byteCount = reader.ReadUInt32();");
        sb.Append(indent).AppendLine("        if (byteCount == 0u) return null;");
        sb.Append(indent).AppendLine("        global::System.ReadOnlySpan<byte> objrefBytes = reader.ReadRawBytes(checked((int)byteCount));");
        sb.Append(indent).AppendLine("        var objrefReader = new global::Opc.Classic.Ndr.NdrReader(objrefBytes);");
        sb.Append(indent).AppendLine("        return global::Opc.Classic.Dcom.OpcInterfaceRefCodec.Read(ref objrefReader);");
        sb.Append(indent).AppendLine("    }");
    }

    private static bool ModelHasInterfacePointerArrays(InterfaceModel model)
    {
        foreach (var method in model.Methods)
        {
            if (method.TaskResultIsOpcInterfaceArray)
            {
                return true;
            }
            foreach (var parameter in method.Parameters)
    ...
    private static void EmitMethods(SourceProductionContext spc, StringBuilder sb, string indent, InterfaceModel model)
    {
        foreach (var method in model.Methods)
        {
            ...
            {
                if (method.InvalidCorrelationReason is not null)
                {
                    spc.ReportDiagnostic(Diagnostic.Create(InvalidCorrelationDescriptor, method.Location, method.Name, method.InvalidCorrelationMember ?? "<unknown>", method.InvalidCorrelationReason));
                }
    ...
    private static void EmitInvokeCoreSignature(StringBuilder sb, string indent, MethodModel method)
    {
        ...
        if (ResponseItemCount(method) > 0) { sb.Append("global::System.Threading.Tasks.Task<").Append(ResponseValueType(method)).Append('>'); } else { sb.Append("global::System.Threading.Tasks.Task"); }
        sb.Append(' ').Append(InvokeCoreName(method)).AppendLine("(");
    }
        ...
        out string? unsupportedParameterName)
    {
        ...
        if (taskReturnKind == TaskReturnKind.TaskOfT &&
            (taskResultMarshallingType is null || (!taskResultIsOpcInterfaceArray && !CanReadType(taskResultMarshallingType, taskResultIsOpcInterface, declaringNamespace))))
        {
        ...
        MethodModel method)
    {
        ...
        if ((parameter.IsUniquePointer || (parameter.IidIsParameterName is not null && IsOpcInterfaceRefType(parameter.MarshallingType))) &&
            TryEmitUniquePointerWrite(sb, statementIndent, writerLocal, parameter, method))
        {
            return;
        }

        // [OpcRefString] on a string parameter: emit a bare conformant-
        // varying string (no leading referent) per DCE 1.1 §4.2.2.7 top-
        // level [ref] pointer semantics.
        if (parameter.RefString && IsStringMarshallingType(parameter.MarshallingType))
        {
            sb.Append(statementIndent).Append(writerLocal).Append(".WriteUnicodeString(").Append(parameter.Name).AppendLine(");");
            return;
        }
        ...
        MethodModel method)
    {
        ...
        {
            sb.Append(statementIndent).Append(writerLocal).Append(".WriteUniquePointerReferent(").Append(parameter.Name).AppendLine(".HasValue);");
        ...
        bool fileTimeElements = false)
    {
        ...
        if (deferredElements && IsStringElementCodec(codec))
        {
            ...
            sb.Append(statementIndent).AppendLine("    {");
            sb.Append(statementIndent).Append("        ").Append(writerLocal).Append(".WriteUniquePointerReferent(").Append(parameterName).Append('[').Append(idxLocal).AppendLine("] is not null);");
            sb.Append(statementIndent).AppendLine("    }");
            sb.Append(statementIndent).Append("    for (int ").Append(idxLocal).Append(" = 0; ").Append(idxLocal).Append(" < ").Append(parameterName).Append(".Length; ").Append(idxLocal).AppendLine("++)");
            sb.Append(statementIndent).AppendLine("    {");
    ...
    private static string? TryGetDeferredPileArrayHelperWrite(string? arrayElementType) => arrayElementType switch
    {
        ...
        "global::Opc.Classic.Dx.DxItemIdentifier" =>
            "global::Opc.Classic.Dx.Ndr.NdrOpcDxItemIdentifierArrayCodec.Write",
        "global::Opc.Classic.Dx.DxSourceServer" =>
            "global::Opc.Classic.Dx.Ndr.NdrOpcDxSourceServerArrayCodec.Write",
        "global::Opc.Classic.Dx.DxConnection" =>
            "global::Opc.Classic.Dx.Ndr.NdrOpcDxConnectionArrayCodec.Write",
        _ => null,
    };
    ...
    private static void EmitCodecReadResponse(
        StringBuilder sb, string indent, string readerLocal, MethodModel method,
        string responsePayloadLocal, string responseSpanLocal)
    {
        string statementIndent = indent + "            ";
        int itemIndex = 0;
        if (method.TaskReturnKind == TaskReturnKind.TaskOfT)
        {
            if (method.TaskResultEnumeratorCountParameterName is not null)
            {
                EmitEnumeratorArrayRead(
                    sb,
                    statementIndent,
                    readerLocal,
                    method,
            ...
            else if (method.TaskResultIsOpcInterfaceArray)
            {
                EmitInterfacePointerArrayRead(sb, statementIndent, readerLocal, method, method.TaskResultType!, method.TaskResultOpcInterfaceArrayElementType!, ResponseValueLocalName(itemIndex), method.TaskResultIsUniquePointer, method.TaskResultIidIsParameterName);
            }
            else
            {
                string? explicitCountLocal = null;
                if (method.TaskResultEmitArrayCount)
                {
                    explicitCountLocal = UniqueLocalName(method.ParameterNames, ResponseValueLocalName(itemIndex) + "ExplicitCount", readerLocal);
                    sb.Append(statementIndent).Append("uint ").Append(explicitCountLocal).Append(" = ").Append(readerLocal).AppendLine(".ReadUInt32();");
                }
                EmitCodecReadLocal(sb, statementIndent, readerLocal, method, method.TaskResultType!, method.TaskResultMarshallingType!, method.TaskResultIsOpcInterface, method.TaskResultIsUniquePointer, ResponseValueLocalName(itemIndex), responsePayloadLocal, responseSpanLocal, method.TaskResultIidIsParameterName);
                if (explicitCountLocal is not null)
                {
                    sb.Append(statementIndent).Append("if (checked((uint)").Append(ResponseValueLocalName(itemIndex)).Append(".Length) != ").Append(explicitCountLocal).AppendLine(")");
                    sb.Append(statementIndent).AppendLine("{");
                    sb.Append(statementIndent).AppendLine("    throw new global::System.IO.InvalidDataException(\"Explicit response count does not match the conformant array count.\");");
                    sb.Append(statementIndent).AppendLine("}");
                }
            }
            itemIndex++;
        ...
        foreach (var parameter in method.Parameters)
        {
            if (!parameter.IsResponseValue) { continue; }
            EmitResponseValueRead(sb, statementIndent, readerLocal, method, parameter, ResponseValueLocalName(itemIndex), responsePayloadLocal, responseSpanLocal);
            itemIndex++;
        }
        EmitResponseCorrelationChecks(sb, statementIndent, method);
        EmitDecodedResponseReturn(sb, statementIndent, method);
    }

    private static void EmitEnumeratorArrayRead(
        StringBuilder sb,
        string statementIndent,
        string readerLocal,
        MethodModel method,
        string targetLocal)
    {
        TryGetCodec(method.TaskResultMarshallingType!, method.DeclaringNamespace, out CodecEmitter codec);
        string elementType = codec.ArrayElementType!;
        string requestedExpression = "checked((int)" + EscapeIdentifier(method.TaskResultEnumeratorCountParameterName!) + ")";
        string maximumLocal = UniqueLocalName(method.ParameterNames, targetLocal + "Maximum", readerLocal, targetLocal);
        string actualLocal = UniqueLocalName(method.ParameterNames, targetLocal + "Actual", readerLocal, targetLocal, maximumLocal);
        string fetchedLocal = UniqueLocalName(method.ParameterNames, targetLocal + "Fetched", readerLocal, targetLocal, maximumLocal, actualLocal);
        string arrayLocal = UniqueLocalName(method.ParameterNames, targetLocal + "Array", readerLocal, targetLocal, maximumLocal, actualLocal, fetchedLocal);
        string indexLocal = UniqueLocalName(method.ParameterNames, targetLocal + "Index", readerLocal, targetLocal, maximumLocal, actualLocal, fetchedLocal, arrayLocal);

        if (method.TaskResultIsUniquePointer)
        {
            string presentLocal = UniqueLocalName(method.ParameterNames, targetLocal + "Present", readerLocal, targetLocal, maximumLocal, actualLocal, fetchedLocal, arrayLocal, indexLocal);
            sb.Append(statementIndent).Append("bool ").Append(presentLocal).Append(" = ").Append(readerLocal).AppendLine(".TryReadReferentId(out _);");
            sb.Append(statementIndent).Append("int ").Append(maximumLocal).AppendLine(" = 0;");
            sb.Append(statementIndent).Append("int ").Append(actualLocal).AppendLine(" = 0;");
            sb.Append(statementIndent).Append(elementType).Append("[] ").Append(arrayLocal).AppendLine(" = global::System.Array.Empty<" + elementType + ">();");
            sb.Append(statementIndent).Append("if (").Append(presentLocal).AppendLine(")");
            sb.Append(statementIndent).AppendLine("{");
            EmitEnumeratorArrayBodyRead(sb, statementIndent + "    ", readerLocal, method, codec, requestedExpression, maximumLocal, actualLocal, arrayLocal, indexLocal);
            sb.Append(statementIndent).AppendLine("}");
        }
        else
        {
            sb.Append(statementIndent).Append("int ").Append(maximumLocal).AppendLine(";");
            sb.Append(statementIndent).Append("int ").Append(actualLocal).AppendLine(";");
            sb.Append(statementIndent).Append(elementType).Append("[] ").Append(arrayLocal).AppendLine(";");
            EmitEnumeratorArrayBodyRead(sb, statementIndent, readerLocal, method, codec, requestedExpression, maximumLocal, actualLocal, arrayLocal, indexLocal);
        }

        sb.Append(statementIndent).Append("int ").Append(fetchedLocal).Append(" = checked((int)").Append(readerLocal).AppendLine(".ReadUInt32());");
        sb.Append(statementIndent).Append("if (").Append(fetchedLocal).Append(" != ").Append(actualLocal).AppendLine(")");
        sb.Append(statementIndent).AppendLine("{");
        sb.Append(statementIndent).AppendLine("    throw new global::System.IO.InvalidDataException(\"Enumerator pceltFetched does not match the array actual count.\");");
        sb.Append(statementIndent).AppendLine("}");
        sb.Append(statementIndent).Append("var ").Append(targetLocal).Append(" = ").Append(arrayLocal).AppendLine(";");
    }

    private static void EmitEnumeratorArrayBodyRead(
        StringBuilder sb,
        string statementIndent,
        string readerLocal,
        MethodModel method,
        ...
        string indexLocal)
    {
        sb.Append(statementIndent).Append(maximumLocal).Append(" = checked((int)").Append(readerLocal).AppendLine(".ReadUInt32());");
        if (method.TaskResultEnumeratorIsVarying)
        {
            string offsetLocal = UniqueLocalName(method.ParameterNames, arrayLocal + "Offset", readerLocal, maximumLocal, actualLocal, arrayLocal, indexLocal);
            sb.Append(statementIndent).Append("int ").Append(offsetLocal).Append(" = checked((int)").Append(readerLocal).AppendLine(".ReadUInt32());");
            sb.Append(statementIndent).Append(actualLocal).Append(" = checked((int)").Append(readerLocal).AppendLine(".ReadUInt32());");
            sb.Append(statementIndent).Append("if (").Append(offsetLocal).Append(" != 0 || ").Append(actualLocal).Append(" > ").Append(maximumLocal)
                .Append(" || ").Append(maximumLocal).Append(" != ").Append(requestedExpression).AppendLine(")");
            sb.Append(statementIndent).AppendLine("{");
            sb.Append(statementIndent).AppendLine("    throw new global::System.IO.InvalidDataException(\"Invalid conformant-varying enumerator array bounds.\");");
            sb.Append(statementIndent).AppendLine("}");
        }
        else
        {
            sb.Append(statementIndent).Append(actualLocal).Append(" = ").Append(maximumLocal).AppendLine(";");
            sb.Append(statementIndent).Append("if (").Append(actualLocal).Append(" > ").Append(requestedExpression).AppendLine(")");
            sb.Append(statementIndent).AppendLine("{");
            sb.Append(statementIndent).AppendLine("    throw new global::System.IO.InvalidDataException(\"Enumerator array exceeds the requested count.\");");
            sb.Append(statementIndent).AppendLine("}");
        }

        if (string.Equals(codec.ArrayElementType, "global::Opc.Classic.Batch.OpcBatchSummary", System.StringComparison.Ordinal))
        {
            sb.Append(statementIndent).Append(arrayLocal)
                .Append(" = global::Opc.Classic.Batch.Ndr.NdrOpcBatchSummaryCodec.ReadConformantArrayBody(ref ")
                .Append(readerLocal).Append(", ").Append(actualLocal).AppendLine(");");
            return;
        }

        sb.Append(statementIndent).Append(arrayLocal).Append(" = new ").Append(codec.ArrayElementType).Append('[').Append(actualLocal).AppendLine("];");
        sb.Append(statementIndent).Append("for (int ").Append(indexLocal).Append(" = 0; ").Append(indexLocal).Append(" < ").Append(actualLocal).Append("; ").Append(indexLocal).AppendLine("++)");
        sb.Append(statementIndent).AppendLine("{");
        sb.Append(statementIndent).Append("    ").Append(arrayLocal).Append('[').Append(indexLocal).Append("] = ")
            .Append(FormatReadExpression(codec, readerLocal)).AppendLine(";");
        sb.Append(statementIndent).AppendLine("}");
    }

    private static void EmitResponseValueRead(
        StringBuilder sb, string statementIndent, string readerLocal, MethodModel method,
        ParameterModel parameter, string targetLocal,
        string responsePayloadLocal, string responseSpanLocal)
    {
        ...
        EmitCodecReadLocal(sb, statementIndent, readerLocal, method,
            parameter.DeclaredType, parameter.MarshallingType,
            parameter.IsOpcInterface, parameter.IsUniquePointer,
            targetLocal, responsePayloadLocal, responseSpanLocal, parameter.IidIsParameterName);
    }
        ...
        string? iidIsParameterName)
    {
        ...
        string interfaceIdExpr = iidIsParameterName is null ? nonNullableType + ".InterfaceId" : EscapeIdentifier(iidIsParameterName);
...
#pragma warning disable MA0051 // Two-pass interface pointer array emission is intentionally contiguous.
        ...
        bool isVarying = false)
    {
        ...
        sb.Append(statementIndent).Append(declaredType).Append(' ').Append(targetLocal).AppendLine(";");
        if (isUniquePointer)
        {
            sb.Append(statementIndent).Append("if (!").Append(readerLocal).AppendLine(".TryReadReferentId(out _))");
            sb.Append(statementIndent).AppendLine("{");
            sb.Append(statementIndent).Append("    ").Append(targetLocal).Append(" = global::System.Array.Empty<").Append(elementType).AppendLine(">();");
            sb.Append(statementIndent).AppendLine("}");
            sb.Append(statementIndent).AppendLine("else");
            sb.Append(statementIndent).AppendLine("{");
            statementIndent += "    ";
        ...
        sb.Append(statementIndent).Append("for (int ").Append(indexLocal).Append(" = 0; ").Append(indexLocal).Append(" < ").Append(countLocal).Append("; ").Append(indexLocal).AppendLine("++)");
        sb.Append(statementIndent).AppendLine("{");
        sb.Append(statementIndent).Append("    if (!").Append(referentsLocal).Append('[').Append(indexLocal).AppendLine("])");
        sb.Append(statementIndent).AppendLine("    {");
        sb.Append(statementIndent).Append("        ").Append(arrayLocal).Append('[').Append(indexLocal).AppendLine("] = null!;");
        sb.Append(statementIndent).AppendLine("        continue;");
        sb.Append(statementIndent).AppendLine("    }");
        string objRefLocal = targetLocal + "ObjRef";
        sb.Append(statementIndent).Append("    var ").Append(objRefLocal).Append(" = ReadMInterfacePointerBody(ref ").Append(readerLocal).AppendLine(");");
        if (rawInterfaceRefs)
        {
            sb.Append(statementIndent).Append("    ").Append(arrayLocal).Append('[').Append(indexLocal).Append("] = ").Append(objRefLocal).AppendLine("!;");
        }
        else
        {
            string channelLocal = targetLocal + "Channel";
            sb.Append(statementIndent).Append("    var ").Append(channelLocal).Append(" = global::Opc.Classic.Dcom.OpcSubProxyHelper.RegisterAndYieldChannel(_channel, ").Append(interfaceIdExpression).Append(", ").Append(objRefLocal).AppendLine(");");
            sb.Append(statementIndent).Append("    ").Append(arrayLocal).Append('[').Append(indexLocal).Append("] = ").Append(channelLocal).Append(" is null ? null! : new ").Append(nonNullableElementType).Append("ClientProxy(").Append(channelLocal).AppendLine(");");
        }
        sb.Append(statementIndent).AppendLine("}");
        sb.Append(statementIndent).Append(targetLocal).Append(" = ").Append(arrayLocal).AppendLine(";");
        if (isUniquePointer)
        {
            statementIndent = statementIndent.Substring(0, statementIndent.Length - 4);
            sb.Append(statementIndent).AppendLine("}");
        }
    }
#pragma warning restore MA0051
        ...
        string? iidIsParameterName)
    {
        ...
        {
            sb.Append(statementIndent).Append("var ").Append(targetLocal)
                .Append(" = global::Opc.Classic.Da.Ndr.NdrOpcItemStateCodec.ReadUniqueConformantArray(ref ")
                .Append(readerLocal).AppendLine(");");
            return;
        }
        ...
        if (isOpcInterface || IsOpcInterfaceRefType(marshallingType))
        {
            if (isOpcInterface)
            {
                EmitOpcInterfaceSubProxyRead(sb, statementIndent, readerLocal, declaredType, targetLocal, iidIsParameterName);
                return;
            }
        ...
        bool declareTarget = true)
    {
        ...
        // must be decoded with the deferred-pointer pile layout (DCE 1.1
        // §14.3.12.3). When such a helper is available, route the array
        // decode through it instead of the per-element flat loop. The
        // helper handles the inner unique-pointer referent skip + max_count
        // read + N inline parts + N deferred parts.
        string? deferredHelper = TryGetDeferredPileArrayHelper(codec.ArrayElementType);
    ...
    /// <summary>
    /// Returns a fully-qualified static method name for decoding the
    /// conformant array of <paramref name="arrayElementType"/> using the
    /// deferred-pointer pile layout (DCE 1.1 §14.3.12.3), or
    /// <see langword="null"/> when the element type uses the simple flat
    /// layout. The returned method must have the signature
    /// <c>(ref NdrReader reader, int count) -&gt; T[]</c>.
    /// </summary>
    private static string? TryGetDeferredPileArrayHelper(string? arrayElementType) => arrayElementType switch
    {
        ...
        "global::Opc.Classic.Dx.DxSourceServer" =>
            "global::Opc.Classic.Dx.Ndr.NdrOpcDxSourceServerArrayCodec.ReadUnique",
        "global::Opc.Classic.Dx.DxConnection" =>
            "global::Opc.Classic.Dx.Ndr.NdrOpcDxConnectionArrayCodec.ReadUnique",
        _ => null,
    };
    ...
    private static void EmitRequestCorrelationChecks(StringBuilder sb, string statementIndent, MethodModel method)
    {
        foreach (var parameter in method.Parameters)
        {
            if (parameter.IsRequestValue && parameter.ArrayCountParameterName is { } sourceName)
            {
                EmitArrayCountCheck(sb, statementIndent, parameter.Name, EscapeIdentifier(sourceName), method.Name, parameter.Name);
            }
        }
    }

    private static void EmitResponseCorrelationChecks(StringBuilder sb, string statementIndent, MethodModel method)
    {
        int itemIndex = method.TaskReturnKind == TaskReturnKind.TaskOfT ? 1 : 0;
        if (method.TaskResultArrayCountParameterName is { } taskSource)
        {
            string taskArrayExpression = ResponseValueLocalName(0);
            if (method.TaskResultArrayCountMemberName is { } memberName)
            {
                taskArrayExpression += "." + EscapeIdentifier(memberName);
            }
            EmitArrayCountCheck(sb, statementIndent, taskArrayExpression, CorrelationSourceExpression(method, taskSource), method.Name, "ReturnValue");
        }
        foreach (var parameter in method.Parameters)
        {
            if (!parameter.IsResponseValue)
            ...
            if (parameter.ArrayCountParameterName is { } sourceName)
            {
                EmitArrayCountCheck(sb, statementIndent, ResponseValueLocalName(itemIndex), CorrelationSourceExpression(method, sourceName), method.Name, parameter.Name);
            }
            itemIndex++;
        }
    }

    private static string CorrelationSourceExpression(MethodModel method, string sourceName)
    {
        if (string.Equals(sourceName, "ReturnValue", System.StringComparison.Ordinal))
        {
            return ResponseValueLocalName(0);
        }
        int itemIndex = method.TaskReturnKind == TaskReturnKind.TaskOfT ? 1 : 0;
        foreach (var parameter in method.Parameters)
        {
            if (string.Equals(parameter.Name.TrimStart('@'), sourceName, System.StringComparison.Ordinal))
            {
                string expression = parameter.IsResponseValue ? ResponseValueLocalName(itemIndex) : parameter.Name;
                return parameter.MarshallingType.EndsWith("[]", System.StringComparison.Ordinal)
                    ? "(" + expression + "?.Length ?? 0)"
                    : expression;
            }
            if (parameter.IsResponseValue)
        ...
        return EscapeIdentifier(sourceName);
    }

    private static void EmitArrayCountCheck(StringBuilder sb, string statementIndent, string arrayExpression, string countExpression, string methodName, string memberName)
    {
        sb.Append(statementIndent).Append("if (").Append(arrayExpression).Append(".Length != checked((int)").Append(countExpression).AppendLine("))");
        sb.Append(statementIndent).AppendLine("{");
        sb.Append(statementIndent).Append("    throw new global::System.IO.InvalidDataException(\"").Append(methodName).Append(" response count does not match ").Append(memberName).AppendLine(".\");");
        sb.Append(statementIndent).AppendLine("}");
    }
    ...
    private static bool MethodHasOpcInterfaceOutput(MethodModel method)
    {
        ...
        if (method.TaskResultIsOpcInterfaceArray && method.TaskResultOpcInterfaceArrayElementType is { } taskElement && !IsOpcInterfaceRefType(taskElement))
        {
            return true;
        }
        ...
        foreach (var parameter in method.Parameters)
        {
            ...
            if (parameter.IsResponseValue && parameter.IsOpcInterfaceArray && parameter.OpcInterfaceArrayElementType is { } element && !IsOpcInterfaceRefType(element))
            {
                return true;
            }
            if (parameter.IsResponseValue && parameter.ArrayCountParameterName is { } countSource && IsRequestOnlyParameter(method, countSource))
            {
                return true;
            }
        }
        if (method.TaskResultArrayCountParameterName is { } taskCountSource && IsRequestOnlyParameter(method, taskCountSource))
        {
            return true;
        }
        if (method.TaskResultEnumeratorCountParameterName is { } enumeratorCountSource &&
            IsRequestOnlyParameter(method, enumeratorCountSource))
        {
            return true;
        }
    ...
    private static bool IsRequestOnlyParameter(MethodModel method, string sourceName)
    {
        foreach (var parameter in method.Parameters)
        {
            if (string.Equals(parameter.Name.TrimStart('@'), sourceName, System.StringComparison.Ordinal))
            {
                return parameter.IsRequestValue && !parameter.IsResponseValue;
            }
        }
        return false;
    }
    ...
    private static string GetMultiOutRecordName(IMethodSymbol method)
    {
        foreach (var attr in method.GetAttributes())
        {
            string? fullName = attr.AttributeClass?.ToDisplayString(TypeDisplayFormat)?.Replace("global::", string.Empty);
            if (string.Equals(fullName, OpcGenerateMultiOutRecordAttributeFullName, System.StringComparison.Ordinal) &&
                ...
                attr.ConstructorArguments[0].Value is string recordName &&
    ...
    private static bool HasAttribute(IMethodSymbol method, string attributeFullName)
    {
        foreach (var attr in method.GetAttributes())
        {
            if (attr.AttributeClass is not null &&
    ...
    {
        public ParameterModel(string name, string resultMemberName, string declaredType, string marshallingType, bool isCancellationToken, RefKind refKind, bool isOpcInterface, bool isUniquePointer = false, string? underlyingValueType = null, bool emitArrayCount = false, bool deferredElements = false, bool fileTimeElements = false, bool variantElements = false, bool refString = false, string? iidIsParameterName = null, string? arrayCountParameterName = null, bool isOpcInterfaceArray = false, string? opcInterfaceArrayElementType = null) { Name = name; ResultMemberName = resultMemberName; DeclaredType = declaredType; MarshallingType = marshallingType; IsCancellationToken = isCancellationToken; RefKind = refKind; IsOpcInterface = isOpcInterface; IsUniquePointer = isUniquePointer || isOpcInterface || (iidIsParameterName is not null && IsOpcInterfaceRefType(marshallingType)); UnderlyingValueType = underlyingValueType; EmitArrayCount = emitArrayCount; DeferredElements = deferredElements; FileTimeElements = fileTimeElements; VariantElements = variantElements; RefString = refString; IidIsParameterName = iidIsParameterName; ArrayCountParameterName = arrayCountParameterName; IsOpcInterfaceArray = isOpcInterfaceArray; OpcInterfaceArrayElementType = opcInterfaceArrayElementType; }
        ...
        /// <summary>
        /// True when [OpcFileTimeElements] is present on a long[]: array
        /// elements are serialized as 2x DWORD (4-byte aligned) FILETIME
        /// instead of 8-byte Int64.
        /// </summary>
        public bool FileTimeElements { get; }

        /// <summary>
        /// True when [OpcVariantElements] is present on an OpcVariant[]: array
        /// elements use the MS-OAUT §2.2.29.2 NDR transmission form
        /// (wireVARIANT + pad-to-8 trailer) instead of inline back-to-back
        /// top-level wireVARIANTs.
        /// </summary>
        public bool VariantElements { get; }

        /// <summary>
        /// True when [OpcRefString] is present on a string parameter: the
        /// wire encoding is a bare conformant-varying string (no leading
        /// referent ID). Required for top-level [in, string] LPWSTR per
        /// DCE 1.1 §4.2.2.7 — those are <c>[ref]</c> not <c>[unique]</c>
        /// despite <c>pointer_default(unique)</c> at the interface level.
        /// </summary>
        public bool RefString { get; }
        public string? IidIsParameterName { get; }
        public string? ArrayCountParameterName { get; }
        public bool IsOpcInterfaceArray { get; }
        public string? OpcInterfaceArrayElementType { get; }
    ...
    private sealed class MethodModel
    {
        public MethodModel(
            ...
            string multiOutRecordName,
            TaskReturnKind taskReturnKind,
            string? taskResultType,
            string? taskResultMarshallingType,
            bool taskResultIsOpcInterface,
            bool taskResultIsOpcInterfaceArray,
            string? taskResultOpcInterfaceArrayElementType,
            bool taskResultIsUniquePointer,
            string? taskResultIidIsParameterName,
            string? taskResultArrayCountParameterName,
            string? taskResultArrayCountMemberName,
            bool taskResultEmitArrayCount,
            string? taskResultEnumeratorCountParameterName,
            bool taskResultEnumeratorIsVarying,
            ...
            string? invalidCorrelationMember,
            string? invalidCorrelationReason,
            ...
            Location location)
        {
            ...
            MultiOutRecordName = multiOutRecordName;
            TaskReturnKind = taskReturnKind;
            ...
            TaskResultIsOpcInterfaceArray = taskResultIsOpcInterfaceArray;
            TaskResultOpcInterfaceArrayElementType = taskResultOpcInterfaceArrayElementType;
            TaskResultIsUniquePointer = taskResultIsUniquePointer;
            TaskResultIidIsParameterName = taskResultIidIsParameterName;
            TaskResultArrayCountParameterName = taskResultArrayCountParameterName;
            TaskResultArrayCountMemberName = taskResultArrayCountMemberName;
            TaskResultEmitArrayCount = taskResultEmitArrayCount;
            TaskResultEnumeratorCountParameterName = taskResultEnumeratorCountParameterName;
            TaskResultEnumeratorIsVarying = taskResultEnumeratorIsVarying;
            ...
            InvalidCorrelationMember = invalidCorrelationMember;
            InvalidCorrelationReason = invalidCorrelationReason;
            IsFullyMarshallable = taskReturnKind != TaskReturnKind.Unsupported && unsupportedMarshallingType is null && unsupportedRefOutMarshallingType is null && invalidCorrelationReason is null;
        ...
        public string MultiOutRecordName { get; }
        ...
        public bool TaskResultIsOpcInterfaceArray { get; }
        public string? TaskResultOpcInterfaceArrayElementType { get; }
        /// <summary>
        ...
        public string? TaskResultIidIsParameterName { get; }
        public string? TaskResultArrayCountParameterName { get; }
        public string? TaskResultArrayCountMemberName { get; }
        public bool TaskResultEmitArrayCount { get; }
        public string? TaskResultEnumeratorCountParameterName { get; }
        public bool TaskResultEnumeratorIsVarying { get; }
        ...
        public string? InvalidCorrelationMember { get; }
        public string? InvalidCorrelationReason { get; }
