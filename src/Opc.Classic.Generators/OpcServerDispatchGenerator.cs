...
";
        ...
        out string? invalidReason)
    {
        ...
        if (taskResultArrayCountParameterName is not null &&
            !ValidateArrayCorrelation(method, taskResultArrayCountParameterName, IsTaskResultArray(method), false, taskReturnKind, out invalidReason))
        {
            invalidMember = "ReturnValue";
        }
    }
    ...
    private static bool ValidateArrayCorrelation(IMethodSymbol method, string sourceName, bool isArray, bool targetIsRequestValue, TaskReturnKind taskReturnKind, out string? reason)
    {
        ...
        if (!IsCountType(source.Type))
        {
            reason = $"array-count source '{sourceName}' must be System.Int32 or System.UInt32";
            return false;
        }
    ...
    private static void Emit(SourceProductionContext spc, InterfaceModel? maybeModel)
    {
        ...
        sb.AppendLine();
        sb.Append(indent).Append("    public ").Append(dispatcherName).Append('(').Append(model.FullyQualifiedName).Append(" impl");
        if (hasInterfacePointerOutputs)
        {
            sb.AppendLine(",");
            sb.Append(indent).AppendLine("        global::System.Func<global::System.Guid, object, global::Opc.Classic.Dcom.IOpcInterfaceRef?>? interfaceReferenceResolver = null)");
        }
        else
        {
            sb.AppendLine(")");
    ...
    private static void EmitResponseCorrelationChecks(StringBuilder sb, string statementIndent, MethodModel method)
    {
        ...
        {
            EmitArrayCountCheck(sb, statementIndent, ReturnValueLocal(method), ResponseCorrelationSourceExpression(method, taskSource), method.Name, "ReturnValue");
        }
        foreach (var parameter in method.Parameters)
        {
