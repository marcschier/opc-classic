...
The message wrappers are self-contained and do not delegate to `Opc.Classic.Dcom.Common.Ntlm.Std`; response cryptography lives in `Responses` and the in-tree crypto primitives. MIC, channel-binding, SPNEGO MIC provider, and message-signature helpers are part of the same audit surface.
