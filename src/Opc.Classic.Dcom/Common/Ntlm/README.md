...
The message wrappers are self-contained and do not delegate to `SharpCifs.Std`; response cryptography lives in `src\Opc.Classic.Dcom\rpc\Auth\Responses.cs` and the in-tree crypto primitives. MIC, channel-binding, SPNEGO MIC provider, and message-signature helpers are part of the same audit surface.
