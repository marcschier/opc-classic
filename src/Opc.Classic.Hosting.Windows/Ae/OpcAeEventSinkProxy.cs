...
/// <summary>
/// Outbound Windows COM proxy for a client-supplied <c>IOPCEventSink</c>.
/// </summary>
...
public sealed unsafe class OpcAeEventSinkProxy : IOPCEventSink, IDisposable
{
    ...
    /// <summary>
    /// Calls <c>IOPCEventSink::OnEvent</c> (opnum 3).
    /// </summary>
    ...
    /// <summary>
    /// Releases the held <c>IOPCEventSink</c> pointer.
    /// </summary>
    public void Dispose()
    {
        IntPtr sinkPtr;
        ...
        private static void WriteSafeArrayData(IntPtr dataBuffer, OpcSafeArray array, ushort baseVt, uint elementSize)
        {
            ...
            for (int i = 0; i < data.Length; i++)
            {
                ...
                switch (baseVt)
                {
                    ...
                    case VT_BSTR:
                        Marshal.WriteIntPtr(slot, value is string text ? Marshal.StringToBSTR(text) : IntPtr.Zero); break;
                    default:
                        break;
                }
            }
