...
{
    /// <summary>
    /// The empty variant (vt = VT_EMPTY).
    /// </summary>
    public static OpcVariant Empty { get; } = new(VarType.VT_EMPTY, null);

    /// <summary>
    /// The SQL-style null variant (vt = VT_NULL).
    /// </summary>
    public static OpcVariant Null { get; } = new(VarType.VT_NULL, null);

    /// <summary>
    /// Constructs a variant with the given type and (boxed) value.
    /// </summary>
    ...
    /// <summary>
    /// The OAUT type discriminator.
    /// </summary>
    public VarType Type { get; init; }

    /// <summary>
    /// The .NET-typed value (boxed). Null when <see cref="Type"/> is <see cref="VarType.VT_EMPTY"/> or <see cref="VarType.VT_NULL"/>.
    /// </summary>
    public object? Boxed { get; init; }

    /// <summary>
    /// True if the variant has no value (<c>VT_EMPTY</c> or <c>VT_NULL</c>).
    /// </summary>
    ...
    /// <summary>
    /// Creates a VT_RECORD variant for a registered record layout.
    /// </summary>
    ...
    /// <summary>
    /// Returns the int8 value if <see cref="Type"/> is <see cref="VarType.VT_I1"/>, else null.
    /// </summary>
    public sbyte? AsInt8() => Type == VarType.VT_I1 ? (sbyte?)Boxed : null;

    /// <summary>
    /// Returns the uint8 value if <see cref="Type"/> is <see cref="VarType.VT_UI1"/>, else null.
    /// </summary>
    public byte? AsUInt8() => Type == VarType.VT_UI1 ? (byte?)Boxed : null;

    /// <summary>
    /// Returns the int16 value if <see cref="Type"/> is <see cref="VarType.VT_I2"/>, else null.
    /// </summary>
    public short? AsInt16() => Type == VarType.VT_I2 ? (short?)Boxed : null;

    /// <summary>
    /// Returns the uint16 value if <see cref="Type"/> is <see cref="VarType.VT_UI2"/>, else null.
    /// </summary>
    public ushort? AsUInt16() => Type == VarType.VT_UI2 ? (ushort?)Boxed : null;

    /// <summary>
    /// Returns the int32 value if <see cref="Type"/> is <see cref="VarType.VT_I4"/>, else null.
    /// </summary>
    public int? AsInt32() => Type == VarType.VT_I4 ? (int?)Boxed : null;

    /// <summary>
    /// Returns the uint32 value if <see cref="Type"/> is <see cref="VarType.VT_UI4"/>, else null.
    /// </summary>
    public uint? AsUInt32() => Type == VarType.VT_UI4 ? (uint?)Boxed : null;

    /// <summary>
    /// Returns the int64 value if <see cref="Type"/> is <see cref="VarType.VT_I8"/>, else null.
    /// </summary>
    public long? AsInt64() => Type == VarType.VT_I8 ? (long?)Boxed : null;

    /// <summary>
    /// Returns the uint64 value if <see cref="Type"/> is <see cref="VarType.VT_UI8"/>, else null.
    /// </summary>
    public ulong? AsUInt64() => Type == VarType.VT_UI8 ? (ulong?)Boxed : null;

    /// <summary>
    /// Returns the float value if <see cref="Type"/> is <see cref="VarType.VT_R4"/>, else null.
    /// </summary>
    public float? AsSingle() => Type == VarType.VT_R4 ? (float?)Boxed : null;

    /// <summary>
    /// Returns the double value if <see cref="Type"/> is <see cref="VarType.VT_R8"/>, else null.
    /// </summary>
    public double? AsDouble() => Type == VarType.VT_R8 ? (double?)Boxed : null;

    /// <summary>
    /// Returns the string value if <see cref="Type"/> is <see cref="VarType.VT_BSTR"/>, else null.
    /// </summary>
    public string? AsString() => Type == VarType.VT_BSTR ? (string?)Boxed : null;

    /// <summary>
    /// Returns the bool value if <see cref="Type"/> is <see cref="VarType.VT_BOOL"/>, else null.
    /// </summary>
    public bool? AsBoolean() => Type == VarType.VT_BOOL ? (bool?)Boxed : null;

    /// <summary>
    /// Returns the date value if <see cref="Type"/> is <see cref="VarType.VT_DATE"/>, else null.
    /// </summary>
    public DateTime? AsDate() => Type == VarType.VT_DATE ? (DateTime?)Boxed : null;

    /// <summary>
    /// Returns the 100-ns FILETIME if <see cref="Type"/> is <see cref="VarType.VT_FILETIME"/>, else null.
    /// </summary>
    public long? AsFileTime() => Type == VarType.VT_FILETIME ? (long?)Boxed : null;

    /// <summary>
    /// Returns the SCODE if <see cref="Type"/> is <see cref="VarType.VT_ERROR"/>, else null.
    /// </summary>
    public int? AsError() => Type == VarType.VT_ERROR ? (int?)Boxed : null;

    /// <summary>
    /// Returns the GUID if <see cref="Type"/> is <see cref="VarType.VT_CLSID"/>, else null.
    /// </summary>
    public Guid? AsClsid() => Type == VarType.VT_CLSID ? (Guid?)Boxed : null;

    /// <summary>
    /// Returns the nested VARIANT when the base type is <see cref="VarType.VT_VARIANT"/>.
    /// </summary>
    public OpcVariant? AsVariant() => VarTypeMask.BaseOf(Type) == VarType.VT_VARIANT ? (OpcVariant?)Boxed : null;

    /// <summary>
    /// Returns the record value when the base type is <see cref="VarType.VT_RECORD"/>.
    /// </summary>
    public OpcRecordValue? AsRecord() => VarTypeMask.BaseOf(Type) == VarType.VT_RECORD ? Boxed as OpcRecordValue : null;

    /// <summary>
    /// Returns the referenced value if this variant carries the VT_BYREF modifier.
    /// </summary>
