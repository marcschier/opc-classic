...
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(OcmHda01_LegacySyncReadRawCodeFix)), Shared]
public sealed class OcmHda01_LegacySyncReadRawCodeFix : CodeFixProvider
{
    private const string Title = "Use ReadRawAsync with CancellationToken";

    public override ImmutableArray<string> FixableDiagnosticIds { get; } =
        ImmutableArray.Create(MigrationDiagnosticDescriptors.LegacySyncReadRaw.Id);
    ...
    private static async Task<Document> ApplyAsync(Document document, InvocationExpressionSyntax invocation, CancellationToken cancellationToken)
    {
        ...
        string replacementText = MigrationCodeFixHelpers.ReceiverText(invocation) +
            ".ReadRawAsync(" + MigrationCodeFixHelpers.ArgumentsWithCancellationToken(invocation.ArgumentList) + ")";
        ...
        root = MigrationCodeFixHelpers.AddUsing(root, "Opc.Classic.Hda");
