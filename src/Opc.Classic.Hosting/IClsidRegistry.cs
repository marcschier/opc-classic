...
{
    /// <summary>
    /// Resolves a CLSID to its server registration.
    /// </summary>
    bool TryResolve(Guid clsid, out OpcClsidRegistration registration);

    /// <summary>
    /// Resolves a ProgID alias to its server registration.
    /// </summary>
    bool TryResolveProgId(string progId, out OpcClsidRegistration registration);

    /// <summary>
    /// Enumerates all registered server-class entries.
    /// </summary>
    IEnumerable<OpcClsidRegistration> Enumerate();

    /// <summary>
    /// Registers or replaces a server-class registration.
    /// </summary>
    void Register(OpcClsidRegistration registration);

    /// <summary>
    /// Removes a server-class registration by CLSID.
    /// </summary>
