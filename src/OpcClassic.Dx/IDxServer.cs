...
public interface IDxServer : IAsyncDisposable
{
    ...
    /// <summary>Retrieve DX server runtime state.</summary>
    ...
    /// <summary>List connections matching the optional name filter (empty = all).</summary>
    Task<IReadOnlyList<DxConnection>> GetConnectionsAsync(
        string? nameFilter = null,
        CancellationToken cancellationToken = default);

    /// <summary>Add a new connection or update an existing one by <see cref="DxConnection.Name"/>.</summary>
    Task<OpcResultId> AddOrUpdateConnectionAsync(
        DxConnection connection,
        CancellationToken cancellationToken = default);

    /// <summary>Remove a connection by name.</summary>
    Task<OpcResultId> RemoveConnectionAsync(
        string connectionName,
        CancellationToken cancellationToken = default);

    /// <summary>Reset (clear) all configured connections and source servers.</summary>
