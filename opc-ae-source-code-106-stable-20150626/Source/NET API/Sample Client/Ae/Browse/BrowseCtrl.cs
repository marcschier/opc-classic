...
namespace Opc.Ae.SampleClient
{
	...
	public class BrowseCtrl : System.Windows.Forms.UserControl
	{
		...
		/// </summary>
		public delegate void NodeSelectedEventHandler(BrowseElement element, bool picked);

		/// <summary>
		/// Displays the area hierarchy for the server.
		/// </summary>
		public void ShowAreas(Opc.Ae.Server server)
		{
			m_server = server;
			...
			// create root node.
			TreeNode root = new TreeNode(m_server.Name);
			...
			BrowseArea(root.Nodes, BrowseType.Area, null);

			// browse top level sources.
			BrowseArea(root.Nodes, BrowseType.Source, null);
		...
		public void ShowEventsAndAreas(Opc.Ae.Server server)
		{
			m_server = server;
			...
			BrowseEvents(events.Nodes, EventType.Simple);
			BrowseEvents(events.Nodes, EventType.Tracking);
			BrowseEvents(events.Nodes, EventType.Condition);
			...
			// browse top level areas.
			BrowseArea(areas.Nodes, BrowseType.Area, null);

			// browse top level sources.
			BrowseArea(areas.Nodes, BrowseType.Source, null);
		...
		{	
			if (typeof(BrowseElement).IsInstanceOfType(node.Tag))
			{
		...
		private void FetchChildren(TreeNode node)
		{
			...
			if (typeof(BrowseElement).IsInstanceOfType(node.Tag))
			{
				...
				{
					BrowseArea(node.Nodes, BrowseType.Area,   element);
					BrowseArea(node.Nodes, BrowseType.Source, element);
				}
			...
			// check for category.
			else if (typeof(Category).IsInstanceOfType(node.Tag))
			{
				node.Nodes.Clear();
				BrowseCategory(node.Nodes, (Category)node.Tag);
			}
		...
		/// </summary>
		private void BrowseEvents(TreeNodeCollection nodes, EventType eventType)
		{
			// fetch categories.
			...
			foreach (Category category in categories)
			{
				...
				if (eventType == EventType.Condition)
				{
		...
		/// </summary>
		private void BrowseArea(TreeNodeCollection nodes, BrowseType browseType, BrowseElement area)
		{
			...
			try
			{
				...
				BrowseElement[] elements = m_server.Browse(
				...
				do
				{
					...
					for (int ii = 0; ii < elements.Length; ii++)
					{
						...
						node.ImageIndex         = (browseType == BrowseType.Area)?Resources.IMAGE_CLOSED_BLUE_FOLDER:Resources.IMAGE_GREEN_SCROLL;
						node.SelectedImageIndex = (browseType == BrowseType.Area)?Resources.IMAGE_OPEN_BLUE_FOLDER:Resources.IMAGE_GREEN_SCROLL;
		...
		/// </summary>
		private void BrowseSource(TreeNodeCollection nodes, BrowseElement source)
		{
			// fetch conditions.
		...
		/// </summary>
		private void BrowseCategory(TreeNodeCollection nodes, Category category)
		{
			// fetch conditions.
		...
		private void ApplyFiltersMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (!typeof(Opc.Ae.Server).IsInstanceOfType(node.Tag) && !typeof(BrowseElement).IsInstanceOfType(node.Tag))
				{
					return;
				}
				...
				if (result)
				{
					...
					if (typeof(BrowseElement).IsInstanceOfType(node.Tag))
					{
						BrowseArea(node.Nodes, BrowseType.Area,   (BrowseElement)node.Tag);
						BrowseArea(node.Nodes, BrowseType.Source, (BrowseElement)node.Tag);
					}
					else
					{
						BrowseArea(node.Nodes, BrowseType.Area,   null);
						BrowseArea(node.Nodes, BrowseType.Source, null);
					}
				}
			}
			catch (Exception exception)
		...
		private void BrowseTV_MouseDown(object sender, System.Windows.Forms.MouseEventArgs e)
		{
			...
			if (typeof(Opc.Ae.Server).IsInstanceOfType(clickedNode.Tag))
			{
			...
			if (typeof(BrowseElement).IsInstanceOfType(clickedNode.Tag))
			{
				if (((BrowseElement)clickedNode.Tag).NodeType == BrowseType.Area)
				{
					ApplyFiltersMI.Enabled  = true;
				}
		...
		{		
			try
			{
				...
				if (typeof(BrowseElement).IsInstanceOfType(BrowseTV.SelectedNode.Tag))
				{
					m_NodeSelected((BrowseElement)BrowseTV.SelectedNode.Tag, false);
				}
			}
			catch (Exception exception)
			{
		...
		private void BrowseTV_DoubleClick(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (typeof(BrowseElement).IsInstanceOfType(BrowseTV.SelectedNode.Tag))
				{
					m_NodeSelected((BrowseElement)BrowseTV.SelectedNode.Tag, true);
				}
			}
			catch (Exception exception)
			{
		...
		{		
			try
			{
				...
				BrowseElement selection = null;

				if (typeof(BrowseElement).IsInstanceOfType(BrowseTV.SelectedNode.Tag))
				{
					selection = (BrowseElement)BrowseTV.SelectedNode.Tag;
				}
				else if (!typeof(Opc.Ae.Server).IsInstanceOfType(BrowseTV.SelectedNode.Tag))
				{
					return;
				}
		...
		{		
			try
			{
				...
				new GetEnabledStateDlg().ShowDialog(m_server, (BrowseElement[])elements.ToArray(typeof(BrowseElement)));
			}
			catch (Exception exception)
			{
				MessageBox.Show(exception.Message);
		...
		{		
			try
			{
				if (typeof(Condition).IsInstanceOfType(BrowseTV.SelectedNode.Tag))
				{
					...
					BrowseElement source = (BrowseElement)BrowseTV.SelectedNode.Parent.Tag;
		...
		{	
			try
			{
				...
				{
					BrowseElement source       = (BrowseElement)BrowseTV.SelectedNode.Parent.Parent.Tag;
