...
The program registers the host, the CLSID registry, a tag catalog, a group registry, a periodic signal generator, and the managed DA server implementation. The repository sample reads `OPC_CLASSIC_SAMPLE_PORT` (default `51300`) and lets `OPC_CLASSIC_LISTEN_ADDRESS` override the full bind address; the same convention is useful for containers and systemd services.
...
builder.Services.AddOpcDaServer<ManagedDaServer>(options =>
{
...
The repository sample `TagTree` shows a Matrikon-style namespace with `Random.*`, `Bucket Brigade.*`, wave tags, and error tags. Current hosting also includes reusable DA browse/property helpers: `IOpcAddressSpace`, `FlatHierarchicalNamespace`, `InMemoryAddressSpace`, `DefaultBrowseServerAddressSpace`, `DefaultBrowse`, `DefaultItemProperties`, and `OpcStandardProperties`. Use those helpers when your server should expose a real browse tree or standard DA properties instead of the conformant empty defaults. Production systems should keep the same separation:
...
DA subscriptions are callback connections. `IOpcDaDataChangePublisher` is the server-side fan-out point. Your implementation produces `OpcDaDataChange` batches; the publisher delivers them to advised `IOPCDataCallback` subscribers. Advanced hosts can also advise an `IOpcDataCallbackSink` directly through `OpcDaGroup.AdviseAsync(IOpcDataCallbackSink, CancellationToken)`, which unifies cross-platform managed DCOM callback fan-out with the Windows CCW path. Keep batches coherent:
