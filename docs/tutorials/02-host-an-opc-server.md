...
The current server-hosting surface is deliberately small: `IOpcDaServer` provides status, group creation/removal, and localized error strings, while follow-up work continues to fill in item-level dispatcher coverage. You can still design your server correctly now: keep tag storage separate from group lifecycle, make HRESULT mapping explicit, model data-change publishing as a server-side stream, and register CLSID/ProgID metadata through the common host infrastructure. The concepts in this article match OPC DA 3.00 and the IDL terminology in `External\Include\opcda.idl`.
...
When packages are published, replace the project references with `Opc.Classic.Core`, `Opc.Classic.Da`, and `Opc.Classic.Hosting` packages.
