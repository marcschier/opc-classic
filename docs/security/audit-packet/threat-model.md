...
The NTLM flow in `docs\security\THREAT_MODEL.md:114-145` has credentials entering the client NTLM auth context; Type1, Type2, and Type3 tokens crossing the network in DCE/RPC bind/auth PDUs; server verification of NT proof, MIC, and CBT; and later signed/sealed request/response PDUs.
...
- Server credential-store boundary: `AuthenticationSource.CreateChallenge` and `AuthenticationSource.Authenticate` bridge verifier code to application credential policy.
