...
| **§5 Security** (authentication, authorization, integrity, privacy) | §5 | ✅ mechanism-neutral NTLMv2, direct Kerberos, and Kerberos-first SPNEGO client/listener authentication through `Opc.Classic.Dcom/rpc/Auth/` + `Opc.Classic.Dcom.Kerberos/` + `Opc.Classic.Dcom/Spnego/` — covered in detail by [`docs/conformance/ms-nlmp.md`](ms-nlmp.md) + [`docs/conformance/ms-kile.md`](ms-kile.md) + [`docs/conformance/ms-spng.md`](ms-spng.md) | ✅ acceptors, MIC, fallback, CBT, principal mapping, integrity/privacy | conformant for implemented mechanisms; external AD validation remains |
