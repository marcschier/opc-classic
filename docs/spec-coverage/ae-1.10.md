...
- **Windows CCW path**: ⚠️ vtables are present; real bodies cover `IOPCEventServer::GetStatus`, `QueryAvailableFilters`, `CreateEventSubscription`, `CreateAreaBrowser`, full `IOPCEventSubscriptionMgt::SetFilter/GetFilter` marshaling, returned-attribute/state methods, and `IOPCEventSink::OnEvent` delivery through the subscription connection point. Many server-side array-heavy query methods still return `E_NOTIMPL`.
...
#### 4. Windows CCW array-heavy AE query and condition methods are vtable-only
...
3. Native-array CCW coverage for category, condition, acknowledgement, and enable-state queries.
