...
- Reads the full spec markdown in `ext/private/docs/`
...
The post-Track-O solution baseline is 0 build warnings / 0 build errors and 2113 passed / 12 skipped / 0 failed across 23 .NET test projects.
...
_Generated by review against `ext/private/docs/OPC-AE-1.10.md` and `src/Opc.Classic.Ae/`._
...
limitation, not a defect in the managed stack**.

- **Root cause (DR33, proven by wire tracing + server-side CCW instrumentation):**
  ...
  `ext/inc/opc_ae_p.c`, that stub marks the AE `LPWSTR` params as
  ...
  intermittently crashes `opcae_ps.dll`, and `AckCondition`'s `[in]` unmarshal
  (two deferred `FC_PP` wstring arrays + a FILETIME array) is rejected by the
  stub before the CCW is entered. The `OPCCONDITIONSTATE` FILETIME members must
  stay 8-byte aligned (a 4-align change deterministically crashes the stub).
- **Why this is not a managed-code defect:** the managed proxy↔dispatcher
  round-trip for these methods passes in `AeEndToEndDispatchTests` and in the
  real-TCP loopback integration tests, i.e. the managed wire format is correct.
  The failure is confined to the third-party native marshaler on the
  native-CCW interop path.
- **Clean alternative (deferred):** host `samples-ae` over the managed
  `OpcServerListener` (as the AE loopback integration tests do) instead of the
  native CCW + `opcae_ps.dll` path; the managed path round-trips these methods
  end-to-end. This is intentionally not done for 1.0.0 to preserve the matrix's
  native-CCW conformance intent.

### Closed Track-O Items and Remaining Gaps
...
**Spec file**: `ext/private/docs/OPC-COMMON-1.10.md`
...
**Overall compliance**: **Projection, codec infrastructure, DA server CPX namespace/property helpers, and BitString support are complete; conversion/filter execution remains server-specific**. Earlier claims that codecs, property constants, namespace helpers, and CPX HRESULTs were missing are stale.
...
The CPX review should no longer describe the implementation as codec- or hosting-helper-blocked. Core CPX parsing, encoding/decoding, BitString support, property IDs, HRESULTs, namespace helpers, and managed DA integration helpers are present and tested. Remaining work is server-specific conversion/filter execution and sample workflows.
...
- **Specification**: `ext/private/docs/OPC-XMLDA-1.01.md` (4914 lines)
...
1. **Windows**. The CTT is a 32-bit native COM application; it has no Linux
   or macOS equivalent.
2. **The vendored CTT installers**, already tracked in `ext/private/ctt/` as six
...
Install the six CTT MSIs from `ext/private/ctt/` (run elevated PowerShell):

```pwsh
$installers = @(
  'ext/private/ctt/OPC CTT Common Modules 2.0.15.msi',
  'ext/private/ctt/OPC CTT DataAccess2 Modules 2.0.22.msi',
  'ext/private/ctt/OPC CTT DataAccess3 Modules 1.0.18.msi',
  'ext/private/ctt/OPC CTT AlarmEvents Modules 1.0.14.msi',
  'ext/private/ctt/OPC CTT Historical DA Modules 1.0.8.msi',
  'ext/private/ctt/OPC CTT XMLDA Modules 1.0.8.msi'
)
...
- `ext/private/ctt/readme.txt` — original OPC Foundation CTT inventory
