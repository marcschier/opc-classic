...
- [x] TestServer registration spec + script alignment (Track BH1+BH2+BH3 — `9d6ed944`): canonical WiX-derived `docs/interop/testserver-registration-spec.md`; `tools/register-testserver.ps1` mirrors the full 8-DLL MSI install order; `tools/grant-testserver-acl.ps1` automates the DCOM Launch/Access ACL grant that is the actual cause of `CO_E_SERVER_EXEC_FAILURE`.
