...
namespace Opc.Hda.SampleClient
{
	...
	public class TrendCreateDlg : System.Windows.Forms.Form
	{
		...
		private Opc.Hda.SampleClient.ItemListCtrl ItemsCTRL;
		...
		private void InitializeComponent()
		{
			...
			this.ItemsCTRL = new Opc.Hda.SampleClient.ItemListCtrl();
		...
		/// </summary>
		private IdentifiedResult[] m_results = null;

		/// <summary>
		/// Prompts the user to select items and specify trend properties.
		/// </summary>
		public Trend ShowDialog(Opc.Hda.Server server)
		{
            ...
            trend.StartTime = new Time("NOW-18MO");
            trend.EndTime = new Time("NOW");

			BrowseCTRL.Browse(m_server, null);
			TrendCTRL.Initialize(trend, RequestType.None);
			ItemsCTRL.Initialize(m_server, (ItemIdentifier[])null);
			...
			if (m_results != null)
			{
				foreach (IdentifiedResult item in m_results)
				{
		...
		/// Prompts the user to select items and specify trend properties.
		/// </summary>
		public ItemIdentifier[] ShowDialog(Opc.Hda.Trend trend)
		{
		...
		private void DoAddItems()
		{
			...
			ItemIdentifier[] items = ItemsCTRL.GetItemIDs(false);
