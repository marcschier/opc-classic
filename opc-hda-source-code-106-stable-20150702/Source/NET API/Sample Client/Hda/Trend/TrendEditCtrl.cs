...
namespace Opc.Hda.SampleClient
{
	...
	public class TrendEditCtrl : System.Windows.Forms.UserControl
	{
		...
		private Opc.Hda.SampleClient.TimeCtrl StartTimeCTRL;
		private Opc.Hda.SampleClient.TimeCtrl EndTimeCTRL;
		...
		private Opc.Hda.SampleClient.ItemTimesCtrl TimestampsCTRL;
		/// <summary> 
		...
		private void InitializeComponent()
		{
			...
			this.StartTimeCTRL = new Opc.Hda.SampleClient.TimeCtrl();
			this.EndTimeCTRL = new Opc.Hda.SampleClient.TimeCtrl();
			...
			this.TimestampsCTRL = new Opc.Hda.SampleClient.ItemTimesCtrl();
		...
		/// </summary>
		public void Initialize(Opc.Hda.Trend trend, RequestType type)
		{
			// set controls to default values.
			NameTB.Text                  = "";
			AggregateSpecifiedCB.Checked = false;
            StartTimeCTRL.Set(new Time("NOW-18MO"));
			StartTimeSpecifiedCB.Checked = true;
            EndTimeCTRL.Set(new Time("NOW"));
			...
			if (trend != null)
			{
				foreach (Aggregate aggregate in trend.Server.Aggregates)
				{
					AggregateCB.Items.Add(aggregate);

					if (aggregate.ID == trend.AggregateID)
					{
				...
				AggregateSpecifiedCB.Checked = trend.AggregateID != AggregateID.NOAGGREGATE;
		...
		/// </summary>
		public void Update(Opc.Hda.Trend trend)
		{
			...
			{
				Aggregate aggregate = (Aggregate)AggregateCB.SelectedItem;

				if (aggregate != null)
				{
					trend.AggregateID = aggregate.ID;
				}
			}
		}
		#endregion
