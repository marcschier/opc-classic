...
namespace Opc.Hda.SampleClient
{
	...
	public class AttributesSelectCtrl : System.Windows.Forms.UserControl
	{
		...
		/// </summary>
		public delegate void AttributePickedEventHandler(Attribute[] attributes);
		...
		/// </summary>
		public delegate void AttributeSelectedEventHandler(Attribute attribute);
		...
		/// </summary>
		public void Initialize(Opc.Hda.Server server, ArrayList excludeList)
		{
		...
		/// </summary>
		public Opc.Hda.Attribute[] GetAttributes(bool selected)
		{
		...
		public int[] GetAttributeIDs(bool selected)
		{
			...
			Opc.Hda.Attribute[] attributes = GetAttributes(selected);
		...
		{
			// attribute.
			if (typeof(Attribute).IsInstanceOfType(item))
			{
		...
		private void ViewMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				Attribute attribute = (Attribute)AttributesLV.SelectedItems[0].Tag;
		...
		private void AddMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				foreach (ListViewItem listItem in AttributesLV.Items)
				{
					if (typeof(Attribute).IsInstanceOfType(listItem.Tag))
					{
                        excludeList.Add(((Attribute)listItem.Tag).ID);
					}
				}

				// prompt user to select attributes.
				Attribute[] attributes = new AttributesSelectDlg().ShowDialog(m_server, excludeList);
		...
		private void AttributesLV_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			if (AttributeSelected != null)
			{
				if (AttributesLV.SelectedItems.Count > 0)
				{
					AttributeSelected((Attribute)AttributesLV.SelectedItems[0].Tag);
				}
				else
				{
