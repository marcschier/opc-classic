...
namespace Opc.Hda.SampleClient
{
	...
	public class AttributeFilterEditDlg : System.Windows.Forms.Form
	{
		...
		private Opc.SampleClient.EnumCtrl OperatorCTRL;
		private Opc.SampleClient.ValueCtrl FilterValueCTRL;
		...
		private void InitializeComponent()
		{
			...
			this.OperatorCTRL = new Opc.SampleClient.EnumCtrl();
			this.FilterValueCTRL = new Opc.SampleClient.ValueCtrl();
		...
		/// </summary>
		public BrowseFilter ShowDialog(Opc.Hda.Server server, BrowseFilter filter)
		{
		...
		/// </summary>
		public BrowseFilter ShowDialog(Opc.Hda.Server server, ArrayList excludeIDs)
		{
			// add valid attribute ids to the combo box.
			AttributeCB.Items.Clear();
			...
			OperatorCTRL.Value       = Operator.Equal;
		...
		/// </summary>
		private BrowseFilter PromptUser()
		{
			while (ShowDialog() == DialogResult.OK)
			{
				try
				{
					Attribute attribute = (Attribute)AttributeCB.SelectedItem;
					...
					filter.Operator    = (Operator)OperatorCTRL.Value;
		...
		private void AttributeCB_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (value == null || value.GetType() != attribute.DataType)
				{
					try
					{
						FilterValueCTRL.Value = Opc.Convert.ChangeType(value, attribute.DataType);
					}
					catch
					{
						FilterValueCTRL.Value = Opc.Convert.ChangeType(null, attribute.DataType);
					}
				}
