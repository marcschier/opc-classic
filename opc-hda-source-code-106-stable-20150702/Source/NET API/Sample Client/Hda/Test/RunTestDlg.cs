...
{	
	...
	public class RunTestDlg : System.Windows.Forms.Form
	{
		...
		/// </summary>
		private Opc.Hda.Test._DataSet m_dataset = null;
		...
		/// </summary>
		public void ShowDialog(Opc.Hda.Server server, string fileName, string itemID)
		{
		...
		private void Run()
		{
			...
			try
			{
				...
				// create the item.
				IdentifiedResult[] results = m_server.CreateItems(new ItemIdentifier[] { new ItemIdentifier(m_itemID) });
				...
				// return items.
				items = new Item[] { new Item(results[0]) };
		...
		private TestData[] LoadDataSet()
		{
			try
			{
				...
				foreach (DataRowView row in m_dataset.TestCases.DefaultView)
				{
					...
					trend.AggregateID      = testcase.AggregateID;
					...
					{
						trend.StartTime = new Time(BASETIME.AddSeconds((double)testcase.StartTime));
					}

					if (testcase.EndTime != Decimal.MinValue)
					{
						trend.EndTime = new Time(BASETIME.AddSeconds((double)testcase.EndTime));
					}
		...
		/// </summary>
		private void ExecuteTest(TestData test, Item[] items)
		{
			try
			{
				// read values from server.
				ItemValueCollection[] actualResults = test.Trend.Read(items);
			...
			{
				test.Actual = new ItemValueCollection();
				test.Actual.ResultID = ResultID.E_FAIL;
		...
		/// </summary>
		private ItemValueCollection GetExpectedResults(_DataSet.TestCase testcase)
		{
			...
			{
				// set expected result.
				values.ResultID = new ResultID(testcase.ResultID, "");
				...
				if (rows != null)
				{
					foreach (_DataSet.ItemValue row in rows)
					{
						// create item value.
						...
						value.Quality          = new Opc.Da.Quality((short)(row.Quality & 0x000FFFF));
						value.HistorianQuality = (Opc.Hda.Quality)Enum.ToObject(typeof(Opc.Hda.Quality), (int)(row.Quality & 0xFFFF0000));
		...
		/// </summary>
		private bool CheckResults(ItemValueCollection expected, ItemValueCollection actual)
		{
			// check result codes.
			if (expected.ResultID.Name.Name != actual.ResultID.Name.Name)
			{
				return false;
			}
		...
		private void UpdateItem(ListViewItem listItem, TestData test)
		{
			...
			{
				listItem.SubItems[ii].Text = Opc.Convert.ToString(GetFieldValue(test, ii));
			}
		...
		private object GetFieldValue(TestData test, int fieldID)
		{
			...
			switch (fieldID)
			{
				...
				case AGGREGATE:           
				{ 
					if (test.Trend.AggregateID != AggregateID.NOAGGREGATE)
					{
						foreach (Aggregate aggregate in m_server.Aggregates)
						{
							if (aggregate.ID == test.Trend.AggregateID)
							{
								return aggregate;
							}
						}
				...
				case RESULT: 
				{
					...
					if (test.Passed)
					{
						return String.Format("Passed ({0}, {1} Values)", test.Expected.ResultID.Name.Name, test.Expected.Count);
					}
					...
					if (test.Actual != null)
					{
						...
						// result code wrong.
						if (test.Actual.ResultID.Name.Name != test.Expected.ResultID.Name.Name)
						{
							return "Unexpected result: " + test.Actual.ResultID;
						}
		...
		private void UpdateTestCase(TestData test)
		{
			...
			test.TestCase.AggregateID      = test.Trend.AggregateID;
			...
			test.TestCase.ResultID         = test.Expected.ResultID.Name.Name;
			...
			{
				_DataSet.ItemValue value = null;

				if (rows != null && ii < rows.Length)
				{
					value = (_DataSet.ItemValue)rows[ii];
				}
				else
				{
		...
		private void AddMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				test.Trend                  = new Trend(m_server);
				test.Trend.Name             = "Test01";
				test.Trend.AggregateID      = AggregateID.NOAGGREGATE;
				test.Trend.StartTime        = new Time(BASETIME);
				test.Trend.EndTime          = new Time(BASETIME);
				test.Trend.MaxValues        = 0;
				test.Trend.IncludeBounds    = false;
				test.Trend.ResampleInterval = 0;

				test.Expected = new ItemValueCollection();
		...
		private void CopyMI_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				copy.Trend    = (Trend)test.Trend.Clone();
				copy.Expected = (ItemValueCollection)test.Expected.Clone();
