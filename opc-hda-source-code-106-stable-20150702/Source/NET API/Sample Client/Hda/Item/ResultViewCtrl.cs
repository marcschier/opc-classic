...
namespace Opc.Hda.SampleClient
{
	...
	public class ResultViewCtrl : System.Windows.Forms.UserControl
	{
		...
		/// </summary>
		public delegate void ResultSelectedEventHandler(ItemIdentifier result);
		...
		/// </summary>
		public void Initialize(ItemIdentifier[] results)
		{
			// initialize controls.
		...
		/// </summary>
		public ItemValueCollection[] AppendValues(ItemValueCollection[] results)
		{
			...
			else
			{
				foreach (ItemValueCollection result in results)
				{
					...
					foreach (ItemValueCollection existingResult in (ICollection)ItemNameCB.Tag)
					{
						if (existingResult.ClientHandle != null && existingResult.ClientHandle.Equals(result.ClientHandle))
						{
							...
							existingResult.ResultID  = result.ResultID;
			...
			if (reinitialize)
			{
				Initialize((ItemIdentifier[])updatedResults.ToArray(typeof(ItemValueCollection)));
			}

			// just update the existing results and force a selection changed event.
			else
			{
				ItemNameCB.Tag = (ItemIdentifier[])updatedResults.ToArray(typeof(ItemValueCollection));
			...
			return (ItemValueCollection[])ItemNameCB.Tag;
		}
		#endregion
		...
		{
			// verify selection.
			ItemIdentifier[] results = (ItemIdentifier[])ItemNameCB.Tag;
			...
			// initialize controls.
			ItemIdentifier result = results[index];
			...
			{
				StartTimeTB.Text = ((IActualTime)result).StartTime.ToString("yyyy-MM-dd HH:mm:ss");
				EndTimeTB.Text   = ((IActualTime)result).EndTime.ToString("yyyy-MM-dd HH:mm:ss");
			}

			// update result.
			if (typeof(IResult).IsInstanceOfType(result))
			{
				ResultTB.Text = ((IResult)result).ResultID.Name.Name;
			}
		...
		private void ItemNameCB_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			// initialize controls.
			ItemIdentifier result = results[ItemNameCB.SelectedIndex];
